...
//            MEDIA_INFO_WRITE_SLOW
    ...
    public IRecorder.RecorderSpec configRecorderSpec(CamcorderProfile profile, String cameraId,
        ...
        if (VALUE_ON.equals(settingManager.getSettingController().queryValue("key_microphone")) && validateMicAvailability()) {
...
/*        if (mApp.isExtranelStorageMount() && mApp.isHasPermissionForSD()
                && "sd".equals(mApp.getSettingValue("key_storagepath","phone",mApp.getAppUi().getCameraId()))) {
            mFilePath = mApp.getCameraDirectorySD() + '/' + mFileName;
        } else {
            mFilePath = mCameraContext.getStorageService().getFileDirectory() + '/' + mFileName;
        }*/
        mFilePath = mCameraContext.getStorageService().getFileDirectory() + '/' + mFileName; // modify for bug 71247
    ...
    private String convertOutputFormatToFileExt(int outputFileFormat) {
        ...
        /*prize-modify-bugid:72862 The video format defaults to .mp4-xiaoping-20190318-start*/
        /*return ".3gp";*/
        return ".mp4";
        /*prize-modify-bugid:72862 The video format defaults to .mp4-xiaoping-20190318-end*/
    }
    ...
    public boolean validateMicAvailability(){
        ...
        /*prize-modify-k6203vs-bug Enter the camera video while the FM is playing in the background. FM will not be suspended-xiaoping-20190426-start*/
        if (SystemProperties.getInt("ro.pri.current.project",0) == 2) {
            available = true;
        }
        /*prize-modify-k6203vs-bug Enter the camera video while the FM is playing in the background. FM will not be suspended-xiaoping-20190426-end*/

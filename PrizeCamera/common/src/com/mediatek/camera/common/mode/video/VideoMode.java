...
import android.view.KeyEvent;//prize-custom the shoot key function-tangan-begin
...
/*prize-modify-bugid:71462 71463 Recording device conflict-xiaoping-20190309-start*/
import android.media.AudioFormat;
import android.media.AudioRecord;
import com.mediatek.camera.R;
/*prize-modify-bugid:71462 71463 Recording device conflict-xiaoping-20190309-end*/
/**
...
public class VideoMode extends CameraModeBase {
    ...
    /*prize-add fixbug:[73087]-huangpengfei-2019-3-19-start*/
    long mKeyDownTime;
    boolean mIsKeyUp = true;
    /*prize-add fixbug:[73087]-huangpengfei-2019-3-19-end*/

    // prize-add by zhangguo bug#73435 stop recording too fast and the video stop failed
    private static final int MIN_VIDEO_DUARTION = 1000;
    private long mStartTime;

    /**
     * enum video state used to indicate the video mode state.
     */
    /*protected*/public enum VideoState {//prize-change-huangpengfei-2018-10-29
        STATE_UNKNOWN,
        STATE_PREVIEW,
        STATE_PRE_RECORDING,
        STATE_RECORDING,
    ...
    public boolean onShutterButtonClick() {
        ...
        switch (getVideoState()) {
            case STATE_PREVIEW:
                /*prize-modify-bugid:71462 71463 Recording device conflict-xiaoping-20190309-start*/
                if (!mVideoHelper.validateMicAvailability()) {
                        if (mRecordingHintInfo == null) {
                            mRecordingHintInfo = new IAppUi.HintInfo();
                            mRecordingHintInfo.mDelayTime = 3000;
                            int resid = mApp.getActivity().getResources().getIdentifier("hint_text_background",
                                    "drawable", mApp.getActivity().getPackageName());
                            mRecordingHintInfo.mBackground = mApp.getActivity().getDrawable(resid);
                            mRecordingHintInfo.mType = IAppUi.HintType.TYPE_AUTO_HIDE;
                            mRecordingHintInfo.mHintText = mApp.getActivity().getString(R.string.other_devices_take_up_microphone_reminders);
                        }
                        int marginTop = (int) mApp.getActivity().getResources().getDimension(R.dimen.exit_app_hint_margintop);
                        mApp.getAppUi().showScreenHint(mRecordingHintInfo,marginTop);
                        mAppUi.applyAllUIEnabled(true);
                    /*prize-modify-bugid:71462 71463 Recording device conflict-xiaoping-20190309-end*/
                } else {
            ...
            case STATE_RECORDING:
            case STATE_PAUSED:
                // prize-modify by zhangguo bug#73435 stop recording too fast and the video stop failed
                if(Math.abs(System.currentTimeMillis() - mStartTime) > MIN_VIDEO_DUARTION){
                    stopRecording();
                }else{
    ...
    /*protected*/public VideoState getVideoState() {//prize-change-huangpengfei-2018-10-29
        mLock.lock();
        try {
            return mVideoState;
        } finally {
    ...
    private void startRecording() {
        ...
        mAppUi.updateCaptureOrVideoState(true,"recording");
    ...
    private void stopRecording() {
        ...
        if (getVideoState() == VideoState.STATE_RECORDING
            ...
            try {
                ...
                /*prize-modify-Camera application should be closed as pressing 2 times back button-xiaoping-20190301-start*/
                mAppUi.updateCaptureOrVideoState(false,"video");
                /*prize-modify-Camera application should be closed as pressing 2 times back button-xiaoping-20190301-end*/
                mRecorder.stop();
                LogHelper.i(TAG, "[stopRecording] media recorder stop - ");
            } catch (RuntimeException e) {
    ...
    private void prepareStartRecording() {
        ...
        mAppUi.setUIVisibility(IAppUi.CAMERA_SWITCHER, View.GONE);
    ...
    private void prepareStopRecording() {
        ...
        /*prize-add fixbug[72016][72056]-huangpengfei-2019-02-27-start*/
        if(mAppUi.getModeItem() == null || mAppUi.getModeItem().mModeTitle != IAppUi.ModeTitle.SLOWMOTION){
            mAppUi.setUIVisibility(IAppUi.CAMERA_SWITCHER, View.VISIBLE);
        }
        /*prize-add fixbug[72016][72056]-huangpengfei-2019-02-27-end*/
        mAppUi.showSavingDialog(null, true);
        mAppUi.applyAllUIVisibility(View.VISIBLE);//prize-tangan-20181016-ui option
        ...
        mCanPauseResumeRecording = false;
        mKeyDownTime = 0;//prize-add fixbug:[73087]-huangpengfei-2019-3-19
    }
    ...
    // zhangguo modify, for slowmotion
    ...
    private View.OnClickListener mStopRecordingListener = new View.OnClickListener() {
        public void onClick(View view) {
            ...
            if(Math.abs(System.currentTimeMillis() - mStartTime) > MIN_VIDEO_DUARTION){
                ...
                LogHelper.d(TAG, "listener stop recording too fast");
            }
        }
    };
    ...
    private void doCameraSelect(String newCameraId) {
        ...
        if (getVideoState() == VideoState.STATE_PREVIEW
            ...
            /*prize-open for model merging-huangpengfei-2019-02-23-start*/
            mCameraDevice.openCamera(mSettingManager, mCameraId, false, mRestrictionProvider);
            /*prize-open for model merging-huangpengfei-2019-02-23-end*/
            /*prize-modify-fix Camera opencamera twice-xiaoping-20181122-end*/
        }
    }
	//prize-custom the shoot key function-tangan-begin
    @Override
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_CAMERA:
                ...
                if ("0".equals(mValue)){
                    ...
                    if("1".equals(android.os.SystemProperties.get("ro.pri_video_takeSnapshot", "0"))){
                        if ((getVideoState() == VideoState.STATE_PAUSED ||
                                getVideoState() == VideoState.STATE_RECORDING) && mCanTakePicture) {
                            mAppUi.animationStart(IAppUi.AnimationType.TYPE_CAPTURE, null);
                            mCameraDevice.updateGSensorOrientation(mApp.getGSensorOrientation());
                        ...
                        /*prize-fix bug[69589]-huangpengfei-2018-12-29-start*/
                        //mIApp.getAppUi().triggerShutterButtonClick(LOWEST_PRIORITY);
                        /*prize-modify fixbug:[73087]-huangpengfei-2019-3-19-start*/
                        if (mIsKeyUp){
                            if (event.getDownTime() - mKeyDownTime >= 2000){
                                onShutterButtonClick();
                                mIsKeyUp = false;
                                mKeyDownTime = event.getDownTime();
                            }
                        }
                        /*prize-modify fixbug:[73087]-huangpengfei-2019-3-19-end*/
                        /*prize-fix bug[69589]-huangpengfei-2018-12-29-end*/
                    }
                    return true;
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_CAMERA:
                if ("0".equals(mValue)){
                    ...
                    mIsKeyUp = true;//prize-add fixbug:[73087]-huangpengfei-2019-3-19
                    return true;
                }else if ("2".equals(mValue)){
                    return false;
                }
	...
	//prize-custom the shoot key function-tangan-end

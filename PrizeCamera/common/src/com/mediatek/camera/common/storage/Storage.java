...
class Storage {
    ...
    /*prize-modify-bugid:71247 Write files to an external SD card using File-xiaoping-20190220-start*/
    static Storage getStorage(IApp app) {
        if (sStorage == null) {
            sStorage = new Storage(app.getActivity());
        }
        sContext = app.getActivity();
        sApp = app;
        return sStorage;
    }
    /*prize-modify-bugid:71247 Write files to an external SD card using File-xiaoping-20190220-end*/
    ...
    private boolean isDefaultPathCanUsed() {
        if (sGetDefaultPath != null) {
            try {
                ...
                if (sApp != null && "sd".equals(sApp.getSettingValue("key_storagepath",defaultvalue,sApp.getAppUi().getCameraId())) && sApp.isExtranelStorageMount() && isHaveExternalSDCard()){
                    /*prize-modify-bugid:72856-Change camera default storage to sd card-xiaoping-20190322-end*/
                    sMountPoint = getExternalStoragePath();
                } else {

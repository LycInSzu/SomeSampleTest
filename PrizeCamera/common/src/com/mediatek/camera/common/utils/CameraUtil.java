...
public final class CameraUtil {
    ...
    public static int getNavigationBarHeight(Activity activity) {
        if (isHasNavigationBar(activity)) {
            ...
            LogHelper.i(TAG,"navigationBarHeight: "+navigationBarHeight);
    ...
    private static Size findBestMatchPanelSize(List<Size> sizes,
        ...
        /*prize-modify-bugid:72298 preview is not clear on cif -xiaoping-20190307-start*/
        if (Math.abs(targetRatio - 1.222) <= 0.01) {
            targetRatio = 1.333;
        }
        /*prize-modify-bugid:72298 preview is not clear on cif -xiaoping-20190307-end*/
        double minDiff;
    ...
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
    public static Size getOptimalPreviewSize(IApp app,
                                             List<Size> sizes,
                                             double previewRatio,
                                             boolean needMatchTargetPanelSize) {
        ...
        if (needMatchTargetPanelSize) {
            optimalSize = findBestMatchPanelSize(
                    sizes, previewRatio, panelWidth, panelHeight,app);
            if (optimalSize != null) {
                return optimalSize;
            }
        }
    ...
    private static Size findBestMatchPanelSize(List<Size> sizes,
                                               double targetRatio, int panelWidth, int panelHeight,IApp app) {
        double minDiff;
        double minDiffMax = Double.MAX_VALUE;
        Size bestMatchSize = null;
        int width = 0;
        for (Size size : sizes) {
            ...
            if (Math.abs(ratio - targetRatio) <= ASPECT_TOLERANCE) {
                ...
                if (app.getAppUi().getModeItem() != null && (app.getAppUi().getModeItem().mModeTitle == IAppUi.ModeTitle.BEAUTY
                    ...
                    if (size.getWidth() > width && size.getWidth() <= 2304 && size.getHeight() <= 2304 && Math.min(size.getWidth(),size.getHeight()) <= 1800) {
                        /*prize-modify-add a larger preview size for 19:9-xiaoping-20190226-end*/
                        bestMatchSize = size;
                        width = size.getWidth();
                    }
                }
    ...
    public static void showErrorInfoAndFinish(Activity activity, int error) {
        ...
        LogHelper.e(TAG,"error: "+error+"errorMessage: "+errorMessage);

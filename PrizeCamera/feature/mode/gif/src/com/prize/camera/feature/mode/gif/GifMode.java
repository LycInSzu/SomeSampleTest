...
public class GifMode extends CameraModeBase implements JpegCallback,DeviceCallback,PreviewSizeCallback,GifMerge.GifCallback,GifViewController.OnGifStopClickListener {
    ...
    private static final int PROCESS_DELAY_TIME = 1;

    private static final int GIF_WIDTH_SCREEN_16_9 = 288;
    private static final int GIF_HEIGHT_SCREEN_16_9 = 512;
    private static final int GIF_WIDTH_SCREEN_4_3 = 360;
    private static final int GIF_HEIGHT_SCREEN_4_3 = 480;
    ...
    protected void takePictrue(){
        ...
        if (mGifMerge == null){
            mGifMerge = new GifMerge(mIApp,path,fileName,FRAME_DELAY / 10,
                    PIC_COUNT, mGifWidth, mGifHeight,GifMode.this, mICameraContext.getLocation(), ((CameraActivity)mIApp.getActivity()).getGSensorOrientation());
    ...
    public void onGifMergeComplete(boolean result, int piccount) {
        ...
        mGifMergedNum += 1;
        mProcessedTime = mGifMergedNum * FRAME_DELAY;//System.currentTimeMillis()-mStartTime;
...
//                mCameraSound.play(MediaActionSound.SHUTTER_CLICK);
    ...
    public void onSaveComplete(boolean success) {
        mICameraContext.getSoundPlayback().play(com.mediatek.camera.common.sound.ISoundPlayback.STOP_VIDEO_RECORDING);
        mIApp.getAppUi().applyAllUIEnabled(true);
        mIApp.getAppUi().onPreviewStarted(mCameraId);
        updateModeDeviceState(MODE_DEVICE_STATE_PREVIEWING);
    }
    ...
    private void setAllUIVisible(){
        if(mIsResumed){
            ...
            mIApp.getAppUi().setCameraSwitchVisible(View.VISIBLE);
            mIApp.getAppUi().applyAllUIEnabled(true);
            mIApp.getAppUi().onPreviewStarted(mCameraId);
            updateModeDeviceState(MODE_DEVICE_STATE_PREVIEWING);
        }
    ...
    private void updateMergeProcess(long time){
        if (mGifViewController != null){
            mGifViewController.updateProgress(mGifMergedNum * FRAME_DELAY);
        }
    }
    ...
    private class MyStatusChangeListener implements StatusMonitor.StatusChangeListener{
        ...
        public void onStatusChanged(String key, String value) {
            ...
            if (mPreviewCallbackOk && KEY_PICTURE_SIZE.equalsIgnoreCase(key)){
    ...
    private void disableAllUIExceptionShutter() {
        ...
        mIApp.getAppUi().setCameraSwitchVisible(View.GONE);
    }

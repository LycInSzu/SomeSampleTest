...
public class FilterDevice2Controller extends Device2Controller implements IFilterDeviceController ,
    ...
    private FilterParameterCreateCallback mParameterCallback;
    ...
    public void takePicture(@Nonnull JpegCallback callback) {
        ...
        if(null != mParameterCallback){
            mParameterCallback.onBuildCreated(builder);
        }
    ...
    private void updateTargetPreviewSize(double ratio) {
        try {
            ...
            for (int i = 0; i < length; i++) {
                if(previewSizes[i].getWidth() * previewSizes[i].getHeight() <= 1440 * 1088){
            ...
            //Size values = CameraUtil.getOptimalPreviewSize(mApp, sizes, ratio, true);
    ...
    public CaptureRequest.Builder getBuilder() {
        try{
            return doCreateAndConfigRequest(Camera2Proxy.TEMPLATE_PREVIEW);
        }catch (Exception e){
            e.printStackTrace();
        }
        return null;
    ...
    public void setParameterCallback(FilterParameterCreateCallback callback){
        mParameterCallback = callback;
    }

    public void applayBuilder(CaptureRequest.Builder builder){

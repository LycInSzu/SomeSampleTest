...
public class FilterMode extends CameraModeBase implements JpegCallback,DeviceCallback, PreviewSizeCallback, IFilterDeviceController.FilterChangeCallback , IFilterDeviceController.FilterParameterCreateCallback {
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext, boolean isFromLaunch) {
        ...
        ((FilterDevice2Controller)mIDeviceController).setParameterCallback(this::onBuildCreated);
    ...
    public void pause(@Nonnull DeviceUsage nextModeDeviceUsage) {
        ...
        if(null != mViewController){
            mViewController.uninitEntry();
            mSurfaceTexutre = null;
        }
    ...
    private void initPortraitView() {
        if(null == mViewController){
            mViewController = new FilterViewController(this::onFilterChanged);
            mViewController.init(mIApp, mCameraId, mICameraContext, mIApp.getAppUi().getFilterIndex());
        }
    }
    ...
    public void onFilterChanged(int index) {
        mIApp.getAppUi().setFilterIndex(index);
    ...
    protected CaptureRequest.Builder getBuilder(){
        if(null == mIDeviceController){
            return null;
        }
        return ((FilterDevice2Controller)mIDeviceController).getBuilder();
    }
    ...
    private void setParameters(CaptureRequest.Builder builder, boolean applay){
        ...
        try{
            builder.set(CaptureRequest.VENDOR_UV_FILTER_ENABLE, 1);
            builder.set(CaptureRequest.VENDOR_UV_FILTER_MODE, mViewController.getSelectIndex());
            LogHelper.d(TAG, "setParameters enable=1 select="+mViewController.getSelectIndex());

            if(applay){
                ((FilterDevice2Controller)mIDeviceController).applayBuilder(builder);
            }
        }catch (Exception e){
            e.printStackTrace();
        }

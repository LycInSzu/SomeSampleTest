    ...
    public boolean updatePictureInfo(int width, int height, int format, int maxImage, int previewWidth, int previewHeight) {
        ...
        synchronized (mImageReaderSync) {
            ...
            if(null == mPreviewImageReader){
                ...
                mPreviewImageReader = ImageReader.newInstance(previewWidth, previewHeight, ImageFormat.YV12, 1);
            ...
            synchronized (mImageReaderSync) {
                try {
                    if (null != mImageCallback) {
                        ...
                        byte[] nv21Data = new byte[mPreviewWidth * mPreviewHeight * 3 / 2];
                        int index = 0;

                        if (null != Plane[0]) {
                            ByteBuffer Ybuffer = Plane[0].getBuffer();
                            int size = Ybuffer.remaining();
                            Ybuffer.get(nv21Data, index, size);
                            index += size;
                        } else {
                            img.close();
                            return;
                        ...
                        if (null != Plane[1]) {
                            ...
                            int size = Ubuffer.remaining();
                            Ubuffer.get(nv21Data, index, size);
                            index += size;
                        } else {
                            img.close();
                            return;
                        ...
                        if (null != Plane[2]) {
                            ...
                            Vbuffer.get(nv21Data, index, Vbuffer.remaining());
                        } else {
                            img.close();
                            return;
                        }

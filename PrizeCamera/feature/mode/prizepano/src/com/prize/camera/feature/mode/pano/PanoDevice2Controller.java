...
public class PanoDevice2Controller extends Device2Controller implements IPanoDeviceController ,
    ...
    private static final float PANO_RATIO = 16f / 9;
    ...
    public void stopPreview() {
        ...
        mCaptureSurface.setCaptureCallback(null);
        abortOldSession();
    }
    ...
    public void setPictureSize(Size defaultSize) {
        ...
        if((float)defaultSize.getWidth() / defaultSize.getHeight() - PANO_RATIO > 0.1){
            ...
            for(String s : picsize){
                ...
                if(null != size && size.length == 2){
                    ...
                    if(Math.abs(w / h - PANO_RATIO) < 0.01 && w * h < 13000000){
                        if(null == lastSize){
        ...
        mIsPictureSizeChanged = mCaptureSurface.updatePictureInfo(defaultSize.getWidth(),
                defaultSize.getHeight(), CAPTURE_FORMAT, CAPTURE_MAX_NUMBER, mPreviewWidth, mPreviewHeight);
    ...
    private void configureSession() {
        ...
        try {
            ...
            Surface previewCallback = mCaptureSurface.getPreviewCallbackSurface();
            if(null != previewCallback){
                surfaces.add(previewCallback);
            }
    ...
    private void updateTargetPreviewSize(double ratio) {
        try {
            ...
            Size values = CameraUtil.getOptimalPreviewSize(mActivity, sizes, PANO_RATIO, true);
    ...
    private CaptureRequest.Builder doCreateAndConfigRequest(int templateType) throws CameraAccessException {
        ...
        if (mCamera2Proxy != null) {
            ...
            if (Camera2Proxy.TEMPLATE_PREVIEW == templateType) {
                ...
                Surface previewCallback = mCaptureSurface.getPreviewCallbackSurface();
                if(null != previewCallback){
                    builder.addTarget(previewCallback);
                }
            }
        }
        return builder;

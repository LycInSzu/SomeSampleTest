...
class SdofPhotoDeviceController extends Device2Controller implements
    ...
    private Builder doCreateAndConfigStillCaptureRequest()
        ...
        if (mCamera2Proxy != null) {
            ...
            setFocusPoint(builder); // zhangguo add for dualcamera
        }
        return builder;
    }
    ...
    private Builder doCreateAndConfigRequest(int templateType) throws CameraAccessException {
        ...
        if (mCamera2Proxy != null) {
            ...
            ThumbnailHelper.configPostViewRequest(builder);
            builder.addTarget(mPreviewSurface);
            //added by tangan-modify watermark rotation-20190417-begin 
             int rotation = CameraUtil.getJpegRotationFromDeviceSpec(
                        Integer.parseInt(mCurrentCameraId), mJpegRotation, mActivity);
              LogHelper.i(TAG, "[doCreateAndConfigRequest] capture: mJpegRotation =" + mJpegRotation +"rotation ="+rotation);
              builder.set(CaptureRequest.JPEG_ORIENTATION, rotation);
            //added by tangan-modify watermark rotation-20190417-end
    ...
    public void doCameraOpened(@Nonnull Camera2Proxy camera2proxy) {
            ...
            try {
                if (CameraState.CAMERA_OPENING == getCameraState()
                    ...
                    configureSession();//prize-add-huangpengfei-20190307
                }
            } catch (RuntimeException e) {
                e.printStackTrace();
            }
    ...
    private void setFocusPoint(CaptureRequest.Builder captureBuilder){
        android.util.Size screenPoint = ((CameraActivity)mActivity).getAppUi().getFocusPoint();
        if(screenPoint == null || (screenPoint.getHeight() == 0 && screenPoint.getHeight() == 0)){
            LogHelper.i("", "setUVPicParameters position is null");
            return;
        }

        android.util.Size previewSize = ((CameraActivity)mActivity).getAppUi().getPreviewSize();
        float ratio = (float) previewSize.getHeight() / ((CameraActivity)mActivity).getAppUi().getSurfaceTextureView().getWidth();

        android.util.Size point = new android.util.Size((int)(screenPoint.getWidth() * ratio), (int)(screenPoint.getHeight() * ratio));

        LogHelper.d(TAG, "setfocuspoint x="+point.getWidth()+" y="+point.getHeight());

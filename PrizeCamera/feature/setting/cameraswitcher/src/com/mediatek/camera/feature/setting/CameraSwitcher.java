...
// zhangguo add for dualcam switch to front camera start
...
// zhangguo add for dualcam switch to front camera end
...
public class CameraSwitcher extends SettingBase {
    ...
    public void init(IApp app,
        ...
        if (numOfCameras > 1) {
           ...
           mAppUi.addCameraSwitch(mSwitcherView);//prize-added by tangan-custom ui
...
// zhangguo add for dualcam switch to front camera start
                    if(FeatureSwitcher.isSupportDualCam()){
                        if(mAppUi.getModeItem() != null && mAppUi.getModeItem().mModeTitle == IAppUi.ModeTitle.APERTURE){
                            LogHelper.d(TAG, "[onClick], enter camera dualcamera mode");
                            QuickSwitcherManager quickSwitcherManager = mAppUi.getmQuickSwitcherManager();
                            View picselfie = quickSwitcherManager.getQuickSwitcher(PicselfieParameterViewController.PICSELFIE_PRIORITY);
                            if(null != picselfie){
                    ...
                    if(mAppUi.getPortraitCallForAiTarget() && mFacing.equals(CAMERA_FACING_BACK) && mAppUi.getModeItem() != null && mAppUi.getModeItem().mModeTitle == IAppUi.ModeTitle.FICSEFILE){
                        mAppUi.selectPluginMode(PhotoModeEntry.class.getName(), false, false);
                    }
// zhangguo add for dualcam switch to front camera end
    ...
    private void switchCameraInNormal() {
        String nextFacing = mFacing.equals(CAMERA_FACING_BACK) ? CAMERA_FACING_FRONT
                : CAMERA_FACING_BACK;
        LogHelper.d(TAG, "[switchCameraInNormal], switch camera to " + nextFacing +",mLiftingCount: "+mLiftingCount);
        ...
        if (FeatureSwitcher.isLiftCameraSupport()) {
            if (System.currentTimeMillis() - mApp.getAppUi().getLifaCamera().getLastLiftingTime() < 2500) {
                mLiftingCount ++;
            } else {
                mLiftingCount = 0;
            }
        }
        if (FeatureSwitcher.isLiftCameraSupport() && "1".equals(newCameraId) && mLiftingCount >= IAppUi.LiftCamera.CONDITIONNUMBER) {
            int marginTop = (int) mApp.getActivity().getResources().getDimension(R.dimen.exit_app_hint_margintop);
            mAppUi.showScreenHint(mFrequentlyOpenCameraInfo,marginTop);
            mAppUi.getLifaCamera().updateLiftCount(mLiftingCount);
            mAppUi.setUIEnabled(IAppUi.CAMERA_SWITCHER,false);
            mLiftingCount = 0;
            /*prize-modify-add front lift camera interaction-xiaoping-20190304-end*/
        } else {
            boolean success = mApp.notifyCameraSelected(newCameraId);

...
class ContinuousShotBase extends SettingBase implements
    ...
    public boolean onShutterButtonLongPressed() {
        ...
        if (ICameraMode.ModeType.PHOTO == mModeType && isReadyForCapture()) {
            ...
            if (mAppUi.getCameraId() == 0 && mAppUi.getModeItem().mModeTitle == IAppUi.ModeTitle.PHOTO &&  "off".equals(mDataStore.getValue("key_hdr","off",getStoreScope()))
                    && "off".equals(mAppUi.getPicsflieValue())
                    && "off".equals(mDataStore.getValue("picsefile_switch","off",getStoreScope()))) {
    ...
    private StatusChangeListener mStatusListener = new StatusChangeListener() {
        ...
        public void onStatusChanged(String key, String value) {
            ...
            if (KEY_FOCUS.equalsIgnoreCase(key) && mIsLongPressed
                ...
                /*prize-modify-bug-bokeh mode,press the camera button to shoot as a random after Click focus -xiaoping-20190423-start*/
                if (mAppUi.getCameraId() == 0 && mAppUi.getModeItem().mModeTitle == IAppUi.ModeTitle.PHOTO &&  "off".equals(mDataStore.getValue("key_hdr","off",getStoreScope()))
                        && "off".equals(mAppUi.getPicsflieValue())
                        && "off".equals(mDataStore.getValue("picsefile_switch","off",getStoreScope()))) {
                    startContinuousShot();
                }
                /*prize-modify-bug-bokeh mode,press the camera button to shoot as a random after Click focus -xiaoping-20190423-end*/
            }
        }
    };
    ...
    protected void onContinuousShotStarted() {
        /*prize-modify-bugid:continuousShot only work at photo mode-xiaoping-20190404-start*/
        if (mAppUi.getCameraId() == 1 || mAppUi.getModeItem().mModeTitle != IAppUi.ModeTitle.PHOTO
                || "on".equals(mDataStore.getValue("key_picsefile","off",getStoreScope()))
                || "on".equals(mDataStore.getValue("picsefile_switch","off",getStoreScope()))){
            LogHelper.e(TAG,"In the case of photo mode or proactive shooting, we blocked this operation");
            onContinuousShotStopped();
            return;
        }
        /*prize-modify-bugid:continuousShot only work at photo mode-xiaoping-20190404-end*/
        ...
        if (FeatureSwitcher.isContinuousShotnumSupport()) {
            MAX_CAPTURE_NUMBER = Integer.parseInt(mDataStore.getValue("key_continuous_shot_num",FeatureSwitcher.getDefaultShouNum(),getStoreScope()));
        }
        ...
        if (SystemProperties.getInt("ro.pri.current.project",0) == 3 || "sd".equals(FeatureSwitcher.getStoragePathDefaultValue())) {
            defaultvalue = "sd";
        }

...
public class FocusCaptureRequestConfigure implements ICameraSetting.ICaptureRequestConfigure, IFocus
    ...
    /*prize-modify-bugid:70713 Click the preview interface and the camera flashes back-xiaoping-20190114-start*/
    private static final String[] mSupportedFocusMode = {"infinity", "auto", "macro", "continuous-video", "continuous-picture"};
    /*prize-modify-bugid:70713 Click the preview interface and the camera flashes back-xiaoping-20190114-end*/
    ...
    /*prize-modify-bugid:72855 Only touch focus can be used in video mode-xiaoping-20190327-start*/
    public FocusCaptureRequestConfigure(Focus focus, ISettingManager.SettingDevice2Requester
            device2Requester, IApp app) {
        mContext = app.getActivity().getApplicationContext();
        mFocus = focus;
        mDevice2Requester = device2Requester;
        mApp = app;
    }
    ...
    public boolean needWaitAfTriggerDone() {
        ...
        LogHelper.i(TAG,"getContinusValue: "+mFocus.getContinusValue()+",flashValue: "+flashValue);
        if (FLASH_OFF_VALUE.equals(flashValue) || "on".equals(mFocus.getContinusValue())) {
            return false;
        }
    ...
    public void updateFocusArea(List<Camera.Area> focusArea, List<Camera.Area> meteringArea) {
        //add for xiaoping 20181225 java.lang.IllegalArgumentException: rect.height must be nonnegative
        if (focusArea != null && focusArea.get(0).rect.height() >= 0 && focusArea.get(0).rect.width() >= 0) {
    ...
    private void sendAutoFocusTriggerCaptureRequest(boolean needCancelAutoFocus) {
        ...
        try {
            if (SLOW_MOTION.equals(mFocus.getCurrentMode())) {
                ...
                sessionProxy.captureBurst(captureRequests, mPreviewCallback, null); // prize add by zhangguo, for bug#74238 can not takepicture after focus
            } else {
                LogHelper.i(TAG, "[sendAutoFocusTriggerCaptureRequest] is common mode");
                sessionProxy.capture(builder.build(), mPreviewCallback, null); // prize add by zhangguo, for bug#74238 can not takepicture after focus
            }
    ...
    private void sendAutoFocusCancelCaptureRequest() {
        ...
        try {
            if (SLOW_MOTION.equals(mFocus.getCurrentMode())) {
                ...
                List<CaptureRequest> captureRequests = null;
                captureRequests = sessionProxy.createHighSpeedRequestList(builder.build());
                sessionProxy.captureBurst(captureRequests, mPreviewCallback, null); // prize modify by zhangguo, for bug#74238 can not takepicture after focus
            } else {
                LogHelper.i(TAG, "[sendAutoFocusCancelCaptureRequest] is common mode");
                sessionProxy.capture(builder.build(), mPreviewCallback, null); // prize modify by zhangguo, for bug#74238 can not takepicture after focus
            }
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
    ...
    private List<String> getAppSupportedFocusModes() {
        ...
        if (mCameraCharacteristics != null) {
            ...
            LogHelper.d(TAG, "[getAppSupportedFocusModes] availableAfModes = "
                    ...
                    appSupportedFocusModeList+",mSupportedFocusMode: ");
    ...
    private void addBaselineCaptureKeysToRequest(CaptureRequest.Builder builder) {
        ...
        /*prize-modify-bugid:72855 Only touch focus can be used in video mode-xiaoping-20190327-start*/
        if (mApp != null && mApp.getAppUi() != null && mApp.getAppUi().getModeItem().mModeTitle == IAppUi.ModeTitle.VIDEO) {
            if (FeatureSwitcher.getCurrentProjectValue() == 3 || FeatureSwitcher.getVideoFocusMode() == 1) {
                if ("recording".equals(((CameraAppUI)mApp.getAppUi()).getCaptureType())) {
                    mCurrentFocusMode = CameraMetadata.CONTROL_AF_MODE_AUTO;
                } else {
                    mCurrentFocusMode = CameraMetadata.CONTROL_AF_MODE_CONTINUOUS_VIDEO;
                }
            } else if (FeatureSwitcher.getCurrentProjectValue() == 4 || FeatureSwitcher.getVideoFocusMode() == 2) {
                    mCurrentFocusMode = CameraMetadata.CONTROL_AF_MODE_MACRO;
            }
        }
        /*prize-modify-bugid:72855 Only touch focus can be used in video mode-xiaoping-20190327-end*/

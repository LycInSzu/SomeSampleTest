...
public class PictureSizeCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        mPictureSize.getAppUi().updateCameraCharacteristics();
        StreamConfigurationMap s = characteristics
                .get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        Size maxPictureSize = characteristics
        ...
        if (supportedSizes != null && maxPictureSize != null) {
            LogHelper.i(TAG,",maxPictureSize.getWidth: "+maxPictureSize.getWidth()+",supportedSizes.get(0).getWidth: "+supportedSizes.get(0).getWidth());
            if (supportedSizes.get(0).getWidth() <= maxPictureSize.getWidth()) {
                mPictureSize.getAppUi().updateCameraCharacteristics();
                s = characteristics
                        .get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
                supportedSizes = getSupportedPictureSize(s, ImageFormat.JPEG);
                sortSizeInDescending(supportedSizes);
            }
        }

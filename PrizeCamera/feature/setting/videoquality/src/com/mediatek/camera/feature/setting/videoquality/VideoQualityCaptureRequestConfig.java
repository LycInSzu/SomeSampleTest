...
public class VideoQualityCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    private String getDefaultQuality() {
        ...
        if (mVideoQuality.getSupportedPlatformValues().size() > QUALITY_INDEX_NUMBER) {
            defaultIndex = mVideoQuality.getSupportedPlatformValues().size() - 1;
        }
        String defaultSize = null;
        defaultSize = FeatureSwitcher.getDefaultVideoSize(Integer.valueOf(mVideoQuality.getCameraId()));
        if (defaultSize == null || defaultSize.isEmpty()) {

            // zhangguo modify for change default video size start
            List<String> videosizes = mVideoQuality.getSupportedPlatformValues();
            ...
            for(String videosize : videosizes){
                ...
                try{
                    curValue = Integer.parseInt(videosize);
                }catch (Exception e){
                    e.printStackTrace();
                }
            ...
            if(maxValue > 0){
                defaultSize = String.valueOf(maxValue);
            }else{
                defaultSize  = mVideoQuality.getSupportedPlatformValues().get(defaultIndex);
            }
            // zhangguo modify for change default video size end

...
public class WatermarkCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    /**
     * Watermark capture request configure constructor.
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        mWatermark.initializeValue(platformSupportedValues, FeatureSwitcher.getBrandWaterDefaultValue());
    }

    @Override
    public void configCaptureRequest(CaptureRequest.Builder captureBuilder) {
        if(captureBuilder != null){
            if (VALUE_ON.equalsIgnoreCase(mWatermark.getValue())) {
                LogHelper.d(TAG, "[configCaptureRequest] Watermark on");
                captureBuilder.set(CaptureRequest.VENDOR_WATERMARK_ENABLE, 1);
            } else {
                LogHelper.d(TAG, "[configCaptureRequest] Watermark off");
                captureBuilder.set(CaptureRequest.VENDOR_WATERMARK_ENABLE, 0);
            }
        }
    }

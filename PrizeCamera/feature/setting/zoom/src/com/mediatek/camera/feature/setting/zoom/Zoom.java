...
public class Zoom extends SettingBase {
    ...
    private static final int MSG_DELAY_FRONT_CAMERA = 60;
    ...
    private class ZoomGestureImpl implements OnGestureListener {
        ...
        public boolean onSingleTapConfirmed(float x, float y) {
            /*prize-modify-add False focus for front camera-xiaoping-20181214-start*/
            LogHelper.i("","isFrontCamera: "+isFrontCamera());
            if (FeatureSwitcher.isFalseFocusSupported() && isFrontCamera() && mZoomConfig != null) {
                mZoomConfig.onScalePerformed(0.03f);
                requestZoom();
                mMainHandler.sendEmptyMessageDelayed(MSG_DELAY_FRONT_CAMERA,300);
                return true;
            }
            /*prize-modify-add False focus for front camera-xiaoping-20181214-end*/
            return false;
    ...
    private class ZoomKeyEventListener implements IApp.KeyEventListener {
        ...
        public boolean onKeyDown(int keyCode, KeyEvent event) {
            ...
            String value = mDataStore.getValue("key_volumekeys", "", getStoreScope());
            if ("0".equals(value)){
                return false;
                ...
                if (keyCode == /*CameraUtil.KEYCODE_ZOOM_IN*/KeyEvent.KEYCODE_VOLUME_UP) {
                    mZoomConfig.onScalePerformed(0.1f);
                    requestZoom();
                    return true;
                    //mMainHandler.obtainMessage(MSG_ZOOM_IN, getTargetRatioMsg(true)).sendToTarget();
                } else if (keyCode == /*CameraUtil.KEYCODE_ZOOM_OUT*/KeyEvent.KEYCODE_VOLUME_DOWN) {
                    mZoomConfig.onScalePerformed(-0.1f);
                    requestZoom();
                    return true;
    ...
    private class MainHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            LogHelper.i("msg.what: ",msg.what+",mZoomConfig: "+mZoomConfig);
            switch (msg.what) {
                case MSG_ZOOM_IN:
                    if (mZoomConfig == null) {
                        return;

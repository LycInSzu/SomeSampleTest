...
public class HorizontalScrollerLayout extends HorizontalScrollView {
    ...
    private void initView() {
    	...
    	LogHelper.i(TAG, "initView count=" + count);
...
//					getPaddingBottom());
    ...
    private void ajustScorllX() {
		...
		for (int i = 0, count = mScrollStrip.getChildCount(); i < count; i++) {
			...
			LogHelper.i(TAG, "ajustScorllX scrollX=" + scrollX + " ajustWidth=" + ajustWidth);
			if (scrollX < ajustWidth) {
				postion = i;
				break;
			}
		}
		LogHelper.i(TAG, "ajustScorllX postion=" + postion);
...
//				- Utils.getMarginStart(selectedTab);
		x -= (first - selected) / 2;
		LogHelper.i(TAG, "scrollToCenter tabIndex=" + tabIndex + " x=" + x);
...
//		scrollTo(x, 0);
    ...
    public boolean onTouchEvent(MotionEvent event) {
		LogHelper.i(TAG, "onTouchEvent event=" + event.getAction() + " getScrollX()=" + getScrollX());

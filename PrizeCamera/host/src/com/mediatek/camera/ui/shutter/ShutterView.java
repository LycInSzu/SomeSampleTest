...
class ShutterView extends RelativeLayout {
    ...
    private final static int THRESHOLD_VALUE = 10;//prize-tangan-20180921-add prizecamera
    ...
    protected void onFinishInflate() {
        ...
        /*prize-remove bug[70904] not allow other ways switch mode-huangpengfei-2019-01-22-start*/
        ...
        /*prize-remove bug[70904] not allow other ways switch mode-huangpengfei-2019-01-22-end*/
    }
    ...
    private void updateShutterView(int scrollX, int parentCenterX, int scrollDistance) {
        ...
        if (scrollDistance == 0) {
            if (diff <= THRESHOLD_VALUE) {
                ...
                /*prize-modify-bug The photo button can also be clicked during the timed photo taking process-xiaoping-20190409-start*/
                if (FeatureSwitcher.getCurrentProjectValue() != 2 && mApp.getAppUi().getSelfTimerState()) {
            ...
            } else if (diff < getMeasuredWidth()) {
                float alpha = 1.0f * diff / getMeasuredWidth();
    ...
    /*prize-modify-camera button disappears as a random-xiaoping-20190419-start*/
    private boolean isSelectedShutter() {
        boolean isSelectedShutter = false;
        if (mApp.getAppUi().getModeItem() != null) {
            isSelectedShutter = ("Video".equals(mType) && "Video".equals(mApp.getAppUi().getModeItem().mType))
                    || ("Picture".equals(mType) && !"Video".equals(mApp.getAppUi().getModeItem().mType));
        }
        return isSelectedShutter;
    }
    /*prize-modify-camera button disappears as a random-xiaoping-20190419-end*/
}

...
public class VideoUI implements IVideoUI {
        ...
        switch (infoId) {
            ...
            case VIDEO_RECORDING_ERROR:
                ...
                if (FeatureSwitcher.getCurrentProjectValue() == 2) {
                    mVideoErrorHint.mHintText = mActivity.getString(R.string.other_devices_take_up_microphone_reminders);
                    int marginTop = (int) mApp.getActivity().getResources().getDimension(R.dimen.exit_app_hint_margintop);
                    mApp.getAppUi().showScreenHint(mVideoErrorHint,marginTop);
                } else {
                    mApp.getAppUi().showScreenHint(mVideoErrorHint);
                }
    ...
    private void updateRecordingViewIcon() {
        ...
        int playPauseId = R.drawable.prize_selector_recording_pause;//prize-modify UI-huangpengfei-2019-01-21
        if (mUIState == VideoUIState.STATE_PAUSE_RECORDING) {
            recordingId = R.drawable.ic_pause_indicator;
            playPauseId = R.drawable.prize_selector_recording_resume;//prize-modify UI-huangpengfei-2019-01-21
        }

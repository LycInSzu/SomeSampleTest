...
import com.pri.factorytest.util.AgingTestItems;
...
import com.pri.factorytest.emmc.ScanExceptionService;//prize-added by tangan-add emmc test-begin

public class AgingTestActivity extends PrizeBaseActivity {
    ...
    /** prize-add by caoshaowei-20190105-begin */
    private CheckBox startFrontMotorChkBox;
    private EditText edtFrontMotorTestCount;
    private TextView prevFrontMotorCount;
    /** prize-add by caoshaowei-20190105-end */
	...
	private CheckBox mDdrTestCheckBox,mEmmcTestCheckBox;
	private TextView mDdrTestInfo,mEmmcTestInfo;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDdrTestCheckBox.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    mApp.getSharePref().putValue("ddr_test_start", "1");
                } else {
                    mApp.getSharePref().putValue("ddr_test_start", "0");
                }
            }
        });
        if ("0".equals(mApp.getSharePref().getValue("ddr_test_start"))) {
            mDdrTestCheckBox.setChecked(false);
        } else {
            mDdrTestCheckBox.setChecked(true);
        }
        //check has the memtester bin is exist on the device
        boolean isMemtesterExist = Utils.isFileExists("/system/bin/memtester");
        String ddrTestStr = mApp.getSharePref().getValue("ddr_test_start");
        boolean isDdrTestSel = "1".equals(ddrTestStr);
        mApp.getSharePref().putValue("ddr_test_start", isMemtesterExist ? (isDdrTestSel ? "1" : "0") : "0");
        findViewById(R.id.ddr_test_select_layout).setVisibility(isMemtesterExist ? View.VISIBLE : View.GONE);
        mDdrTestInfo = (TextView) findViewById(R.id.ddr_test_info);
		//prize-added by tangan-add emmc test-begin
		mEmmcTestCheckBox = (CheckBox) findViewById(R.id.emmc_test_select);
		mEmmcTestCheckBox.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if(isChecked){
					mApp.getSharePref().putValue("emmc_test_start","1");
				}else{
					mApp.getSharePref().putValue("emmc_test_start","0");
				}
			}
		});
		if("1".equals(mApp.getSharePref().getValue("emmc_test_start"))){
			mEmmcTestCheckBox.setChecked(true);
		}else{
			mEmmcTestCheckBox.setChecked(false);
		}
		if(mEmmcTestCheckBox.isChecked()){
			mApp.getSharePref().putValue("emmc_test_start","1");
		}else{
			mApp.getSharePref().putValue("emmc_test_start","0");
		}
		mEmmcTestInfo = (TextView) findViewById(R.id.emmc_test_info);
		//prize-added by tangan-add emmc test-end
        ...
        startButton.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (bReboot) {
                    mApp.getSharePref().putValue("reboot_selected", "1");
                }
                if (bSleep) {
                    mApp.getSharePref().putValue("sleep_selected", "1");
                }
                handleStartTest();
            }
        });
        ...
        HashMap<String, String> houtminute = seconds2hourminute(Long.parseLong(mApp.getSharePref().getValue("parallel_time")));
        ...
        /** prize-add by caoshaowei-20190105-begin */
        startFrontMotorChkBox = (CheckBox) findViewById(R.id.front_motor_chk_box);
        startFrontMotorChkBox.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                mApp.getSharePref().putValue("front_motor_test_start", "1");
            } else {
                mApp.getSharePref().putValue("front_motor_test_start", "0");
            }
        });
        if ("0".equals(mApp.getSharePref().getValue("front_motor_test_start"))) {
            startFrontMotorChkBox.setChecked(false);
        } else {
            startFrontMotorChkBox.setChecked(true);
        }
        boolean isfrontMotorExist = Utils.isFileExists("/proc/hall1/m1120_up") || Utils.isFileExists("/proc/hall2/m1120_down");
        /*String frontMotorStr = mApp.getSharePref().getValue("front_motor_test_start");
        boolean isfrontMotorSel = "1".equals(frontMotorStr);
        mApp.getSharePref().putValue("front_motor_test_start", isfrontMotorExist ? (isfrontMotorSel ? "1" : "0") : "0");*/
        findViewById(R.id.motor_lt).setVisibility(isfrontMotorExist ? View.VISIBLE : View.GONE);

        edtFrontMotorTestCount = (EditText) findViewById(R.id.edt_front_motor_test_count);
        edtFrontMotorTestCount.setText(getFrontMotorTestCount());
        prevFrontMotorCount = (TextView) findViewById(R.id.front_motor_test_count);
        //prevFrontMotorCount.setText(getPrevFrontText());
        /** prize-add by caoshaowei-20190105-end */
    }
    ...
    protected void onResume() {
        initRebootSleep();
        updateDdrTestStatus();
		updateEmmcTestStatus();//prize-added by tangan-add emmc test-begin
        //add liup aging recovery backup
        creatFile();
        readFileValue();
        //add liup aging recovery backup
        prevFrontMotorCount.setText(getPrevFrontText()); /** prize-add by caoshaowei-20190105 */
        super.onResume();
    }
    ...
    private void readFileValue() {
        ...
        String resultReboot = mApp.getSharePref().getValue(KEY_REBOOT_RESULT);
        String resultSleep = mApp.getSharePref().getValue(KEY_SLEEP_RESULT);
        try {
    ...
    private String seconds2minute(String key) {
        String seconds = mApp.getSharePref().getValue(key);
    ...
    private void setTestResult(String key, String result) {
        if (key.equals(KEY_REBOOT_RESULT)) {
            mApp.getSharePref().putValue(key, result);
    ...
    private void handleStartTest() {
        if (hasParallelItem()) {
            mApp.getSharePref().putValue("parallel_time", String.valueOf(getParallelTime()));
        }
        if (hasSleep()) {
            mApp.getSharePref().putValue("sleep_time", String.valueOf(getSleepTime()));
        }
        if (hasReboot()) {
            mApp.getSharePref().putValue("reboot_time", String.valueOf(getRebootTime()));
        }
        /** prize-add by caoshaowei-20190105-begin */
        if (hasFrontMotor()) {
            mApp.getSharePref().putValue("front_motor_test_count", getEdtFrontMotorTestCount());
        }
        /** prize-add by caoshaowei-20190105-end */
        setButton(false);
        resetTestResult();
        if (hasReboot()) {
            mApp.getSharePref().putValue("reboot_currenttimemillis", String.valueOf(System.currentTimeMillis()));
            startReboot();
        } else if (hasSleep()) {
            startSleepTest();
            /** prize-add by caoshaowei-20190105-begin */
        } else if (hasFrontMotor()) {
            startFrontMotorTest();
            /** prize-add by caoshaowei-20190105-end */
        } else if (hasParallelItem()) {
            startParallelTest();
        } else if (!"0".equals(mApp.getSharePref().getValue("ddr_test_start"))) {
		...
		//prize-added by tangan-add emmc test-begin
		}else if("1".equals(mApp.getSharePref().getValue("emmc_test_start"))){
			Intent scanExceptionIntent = new Intent();
			scanExceptionIntent.setClass(this, ScanExceptionService.class);
			scanExceptionIntent.putExtra("startScan", ScanExceptionService.EMCC_TEST_TIMES);
			startService(scanExceptionIntent);
		}
		//prize-added by tangan-add emmc test-end
        else {
            setButton(true);
    ...
    private void handleReboot() {
        long start = Long.parseLong(mApp.getSharePref().getValue("reboot_currenttimemillis"));
        long reboottime = Long.parseLong(mApp.getSharePref().getValue("reboot_time")) * 1000L;
        long currentTime = System.currentTimeMillis();
        boolean isTimeout = currentTime - start > 7 * 24 * 60 * 60 * 1000L;
        if (isTimeout) {
            mApp.getSharePref().putValue("reboot_currenttimemillis", String.valueOf(currentTime));
            start = currentTime;
        }
        if (currentTime - start < reboottime) {
            showDialog();
            ...
            mApp.getSharePref().putValue("reboot_selected", "0");
    ...
    private void startSleepTest() {
        ...
        mSleepCountDownTimer = new CountDownTimer(getSleepTime() * 1000, 3000) {
            ...
            public void onFinish() {
                ...
                mApp.getSharePref().putValue("sleep_selected", "0");
                setTestResult(KEY_SLEEP_RESULT, KEY_PASS_RESULT);
                setButton(true);
                /** prize-modify by caoshaowei-20190105-begin */
                if (hasFrontMotor()) {
                    startFrontMotorTest();
                } else if (hasParallelItem()) {
                    startParallelTest();
                ...
                /** prize-modify by caoshaowei-20190105-end */
            }
     ...
     * prize-add by caoshaowei-20190105-begin
     */
    private String getPrevFrontText() {
        String openMotorCount = mApp.getSharePref().getValue(
                "open_front_motor_count");
        if (openMotorCount == null || openMotorCount.isEmpty()) {
            openMotorCount = "0";
        }
        return getString(R.string.current_front_test_count_str)
                + openMotorCount + getString(R.string.text_count_str);
    }

    private boolean hasFrontMotor() {
        String motor = mApp.getSharePref().getValue("front_motor_test_start");
        if (motor == null || motor.isEmpty() || motor.equals("0"))
            return false;

        if (Integer.parseInt(getEdtFrontMotorTestCount()) < 1) {
            return false;
        }
    ...
    private String getFrontMotorTestCount() {
        String motorCount = mApp.getSharePref().getValue(
                "front_motor_test_count");
        if (motorCount == null || motorCount.isEmpty() || "0".equals(motorCount)) {
            motorCount = getString(R.string.def_front_motor_test_count);
        }
        return motorCount;
    }

    private String getEdtFrontMotorTestCount() {
        String motorCount = edtFrontMotorTestCount.getText().toString();
        if (edtFrontMotorTestCount.getText().toString().length() < 1) {
            motorCount = "0";
        }
        return motorCount;
    }

    private void startFrontMotorTest() {
        mApp.getSharePref().putValue("open_front_motor_count", "0");
        setButton(true);
        Intent intent = new Intent().setClass(
                AgingTestActivity.this, AgingTestImplActivity.class);
        intent.putExtra("START_FRONT_MOTOR_TEST",
                Integer.parseInt(getEdtFrontMotorTestCount()));
        if (hasParallelItem()) {
    ...
    private boolean hasSleep() {
        if (mApp.getSharePref().getValue("sleep_selected").equals("0"))
            return false;
    ...
    private boolean hasReboot() {
        if (mApp.getSharePref().getValue("reboot_selected").equals("0"))
            return false;
    ...
    private boolean hasParallelItem() {
        boolean hasParallel = mApp.getSharePref().getValue("video_speaker_selected").equals("1")
                || mApp.getSharePref().getValue("video_receiver_selected").equals("1")
                || mApp.getSharePref().getValue("vibrate_selected").equals("1")
                || mApp.getSharePref().getValue("mic_loop_selected").equals("1")
                || mApp.getSharePref().getValue("front_camera_selected").equals("1")
                || mApp.getSharePref().getValue("back_camera_selected").equals("1");
        if (!hasParallel) {
            return false;
        }
    ...
    private String getLastDuration() {
        List<String> duraList;
        if (isReadNv) {
            String nvResult = Utils.readProInfo(Utils.PRIZE_AGING_TEST_DURATION, 18);
            String durationNV = nvResult.substring(nvResult.length() - 18);
            //Log.i(TAG, "-----durationNV:" + durationNV);
            duraList = Stream.of(0, 3, 6, 9, 12, 15).map(x ->
                    formatNv2Time(durationNV.substring(x, x + 3))).collect(Collectors.toList());
        } else {
            duraList = Stream.of("video_speaker_duration", "video_receiver_duration",
                    "vibrate_duration", "mic_loop_duration", "front_camera_duration",
                    "back_camera_duration").map(x -> mApp.getSharePref().getValue(x)).collect(Collectors.toList());
        }

        StringBuilder duration = new StringBuilder();
        duration.append(getString(R.string.last_duration_detail) + "\n");
        for (int i = 0; i < 6; i++) {
            duration.append(agingtest_items[i] + ":" + duraList.get(i) + "\n");
        }
        return duration.toString();
    }

    private String formatNv2Time(String nvResult) {
        int nv2Int = formatString2Int(nvResult.trim());
        LocalTime localTime = LocalTime.of(nv2Int / 60, nv2Int % 60, 0);
    ...
    private AgingTestItems loadTransData() {
        ...
        mAgingTestItems.setvideoAndSpeaker(mApp.getSharePref().getValue("video_speaker_selected").equals("1"));
        mAgingTestItems.setvideoAndReceiver(mApp.getSharePref().getValue("video_receiver_selected").equals("1"));
        mAgingTestItems.setVibrate(mApp.getSharePref().getValue("vibrate_selected").equals("1"));
        mAgingTestItems.setMicLoop(mApp.getSharePref().getValue("mic_loop_selected").equals("1"));
        mAgingTestItems.setFrontCamera(mApp.getSharePref().getValue("front_camera_selected").equals("1"));
        mAgingTestItems.setBackCamera(mApp.getSharePref().getValue("back_camera_selected").equals("1"));
    ...
    private void setCheckValue(String[] parallel, String cb, boolean isCheck) {
        ...
        switch (position) {
            case 0:
                mApp.getSharePref().putValue("video_speaker_selected", isCheck ? "1" : "0");
                break;
            case 1:
                mApp.getSharePref().putValue("video_receiver_selected", isCheck ? "1" : "0");
                break;
            case 2:
                mApp.getSharePref().putValue("vibrate_selected", isCheck ? "1" : "0");
                break;
            case 3:
                mApp.getSharePref().putValue("mic_loop_selected", isCheck ? "1" : "0");
                break;
            case 4:
                mApp.getSharePref().putValue("front_camera_selected", isCheck ? "1" : "0");
                break;
            case 5:
                mApp.getSharePref().putValue("back_camera_selected", isCheck ? "1" : "0");
                break;
            default:
                break;
        }
    ...
    private void setCheckValue(CheckBox cb, int position) {
        switch (position) {
            case 0:
                cb.setChecked(mApp.getSharePref().getValue("video_speaker_selected").equals("1"));
                break;
            case 1:
                cb.setChecked(mApp.getSharePref().getValue("video_receiver_selected").equals("1"));
                break;
            case 2:
                cb.setChecked(mApp.getSharePref().getValue("vibrate_selected").equals("1"));
                break;
            case 3:
                cb.setChecked(mApp.getSharePref().getValue("mic_loop_selected").equals("1"));
                break;
            case 4:
                cb.setChecked(mApp.getSharePref().getValue("front_camera_selected").equals("1"));
                break;
            case 5:
                cb.setChecked(mApp.getSharePref().getValue("back_camera_selected").equals("1"));
                break;
            default:
                break;
        }
                ...
                .setPositiveButton(R.string.confirm,
                        new DialogInterface.OnClickListener() {
                            public void onClick(
                                ...
                                intent.putExtra(Intent.EXTRA_WIPE_EXTERNAL_STORAGE, /*mEraseSdCard*/true);
    ...
    private void updateDdrTestStatus() {
        String info = Utils.readProInfo(36, 1);
        info = String.valueOf(info.charAt(info.length() - 1));
        if ("P".equals(info)) {
	...
	//prize-added by tangan-add emmc test-begin
	private void updateEmmcTestStatus() {
		String info = Utils.readProInfo(250, 1);
		info = String.valueOf(info.charAt(info.length() - 1));
		if(ScanExceptionService.EMCC_TEST_SUCCESS.equals(info)){
			mEmmcTestInfo.setTextColor(getResources().getColor(R.color.green));
			mEmmcTestInfo.setText(R.string.pass);
		}else if(ScanExceptionService.EMCC_TEST_FAIL.equals(info)){
			mEmmcTestInfo.setTextColor(getResources().getColor(R.color.red));
			mEmmcTestInfo.setText(R.string.fail);
		}else{
			mEmmcTestInfo.setTextColor(getResources().getColor(R.color.red));
			mEmmcTestInfo.setText(R.string.no_test); 
		}
	}
	//prize-added by tangan-add emmc test-end
}

...
import com.pri.factorytest.util.AgingTestTimer;
import com.pri.factorytest.util.SupportedSizesReflect;
...
public class AgingTestImplActivity extends PrizeBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        keepScreenOn();
        initViews();
        Utils.paddingLayout(findViewById(R.id.video_surface), 0, ACTIVITY_TOP_PADDING, 0, 0);
        mContext = this;
    ...
    private void pickOneEnter(int id, int parameter) {
        ...
        switch (testId) {
			...
			}else if("1".equals(app.getSharePref().getValue("emmc_test_start")) && testId != -10000){
    ...
    private void exitCurrentPickOneEnter(int current, boolean destroy) {
        ...
        switch (currentId) {
            case AgingTestItems.VIDEO_SPEAKER:
                ...
                String videoSpeakerSN = String.format("%3s", String.valueOf(videoSpeakerElapseTime/60000));
                Utils.writeProInfo(videoSpeakerSN, Utils.PRIZE_AGING_TEST_DURATION);
            ...
            case AgingTestItems.VIDEO_RECEIVER:
                ...
                String videoReceiverSN = String.format("%3s", String.valueOf(videoReceiverElapseTime/60000));
                Utils.writeProInfo(videoReceiverSN, Utils.PRIZE_AGING_TEST_DURATION + 3);
                videoSurfaceView.setVisibility(View.GONE);
                setAudio2Speaker();
                pickOneEnter(nextId, NONE_PARAMETER);
                break;
            case AgingTestItems.VIBRATE:
                ...
                String vibrateSN = String.format("%3s", String.valueOf(vibrateElapseTime/60000));
                Utils.writeProInfo(vibrateSN, Utils.PRIZE_AGING_TEST_DURATION + 6);
            ...
            case AgingTestItems.MIC_LOOP:
                ...
                String micSN = String.format("%3s", String.valueOf(micElapseTime/60000));
                Utils.writeProInfo(micSN, Utils.PRIZE_AGING_TEST_DURATION + 9);
                flag = false;
                setAudio2Speaker();
                pickOneEnter(nextId, NONE_PARAMETER);
                break;
            case AgingTestItems.FRONT_CAMERA:
                ...
                String frontCameraSN = String.format("%3s", String.valueOf(frontCameraElapseTime/60000));
                Utils.writeProInfo(frontCameraSN, Utils.PRIZE_AGING_TEST_DURATION + 12);
            ...
            case AgingTestItems.BACK_CAMERA:
                ...
                String backCameraSN = String.format("%3s", String.valueOf(backCameraElapseTime/60000));
                Utils.writeProInfo(backCameraSN, Utils.PRIZE_AGING_TEST_DURATION + 15);
                cameraSurfaceView.setVisibility(View.GONE);
                pickOneEnter(nextId, NONE_PARAMETER);
                break;
            default:
    ...
    private void openCamera() {
        try {
            if (isCameraBack) {
                ...
                //mCamera = Camera.openLegacy(Camera.CameraInfo.CAMERA_FACING_BACK, Camera.CAMERA_HAL_API_VERSION_1_0);
            } else {
                mCamera = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT);
                //mCamera = Camera.openLegacy(Camera.CameraInfo.CAMERA_FACING_FRONT, Camera.CAMERA_HAL_API_VERSION_1_0);
            }
            ...
            if (cameraPosition == 1) {
                if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                    ...
                    try {
                        ...
                        //mCamera = Camera.openLegacy(i, Camera.CAMERA_HAL_API_VERSION_1_0);
                        synchronized (mFaceDetectionSync) {
    ...
    private int FRONTTAKETIME = 1500;

    private void getFrontTestSum() {
        Intent intent = getIntent();
        testSum = intent.getIntExtra("START_FRONT_MOTOR_TEST", 0);
        Log.i("caoshaowei", "testSum : " + testSum);
    }

    /*private Handler mFrontTakeHandler = new Handler();
    private Runnable mFrontTakeRunnable = new Runnable() {
        @Override
        public void run() {
            Log.i("caoshaowei", " ---mFrontTakeRunnable--- " + count);
            takeFrontPicture();
        }
    };*/

    private Handler mFrontExitTakeHandler = new Handler();
    private Runnable mFrontExitTakeRunnable = () -> {
        Log.i("caoshaowei", " ---mFrontExitTakeRunnable--- " + count);
        exitCurrentEnterNextCameraFront();
    };

    private Handler mFrontExitEnterTakeHandler = new Handler();
    private Runnable mFrontExitEnterTakeRunnable = () -> {
        Log.i("caoshaowei", " ---mFrontExitEnterTakeRunnable--- " + count);
        openCameraFront();
    };

    public void openCameraFront() {
        if (count < testSum) {
            mFrontExitEnterTakeHandler.removeCallbacks(mFrontExitEnterTakeRunnable);
            cameraSurfaceView.setVisibility(View.VISIBLE);
            isCameraBack = false;
            cameraPosition = 0;
            ...
            if (count > 0) {
                try {
                    cameraCallback.surfaceCreated(camera_holder);
                    setCameraParameters(getScreenWith(), getScreenHeight());
                    mCamera.setPreviewDisplay(camera_holder);
                    mCamera.startPreview();
                } catch (Exception e) {
                    Log.i("caoshaowei", "openCameraFront - setCameraParameters - Exception : " + e);
                    e.printStackTrace();
                }
            }
            //mFrontTakeHandler.postDelayed(mFrontTakeRunnable, FRONTTAKETIME);
            mFrontExitTakeHandler.postDelayed(mFrontExitTakeRunnable, FRONTTAKETIME);
        }
    }

    private void exitCurrentEnterNextCameraFront() {
        mFrontExitTakeHandler.removeCallbacks(mFrontExitTakeRunnable);
        if (mCamera != null) {
            try {
                cameraSurfaceView.setVisibility(View.GONE);
                cameraCallback.surfaceDestroyed(camera_holder);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        putFrontOpenCount();
        if (count < testSum) {
            mFrontExitEnterTakeHandler.postDelayed(mFrontExitEnterTakeRunnable, FRONTTAKETIME);
        } else {
            camera_holder.removeCallback(cameraCallback);
            mCamera.release();
            gAgingTestItems = getAgingTestItems();
            gDuration = getDuration();
            if (gAgingTestItems != null && gDuration > 0) {
                finish();
                Intent intent = new Intent().setClass(
                        mContext, AgingTestImplActivity.class);
                intent.putExtra("AGINGTEST_ITEMS", gAgingTestItems);
                intent.putExtra("AGINGTEST_DURATION", gDuration);
                mContext.startActivity(intent);
            } else {
                enterDdrTest();
            }
    ...
    /*private void takeFrontPicture() {
        if (mCamera != null) {
            try {
                mCamera.takePicture(mShutterCallback, null, frontJpegCallback);
            } catch (Exception e) {
                Log.i("caoshaowei", " ---takeFrontPicture--- exception : " + e );
                e.printStackTrace();
            }
            mFrontTakeHandler.removeCallbacks(mFrontTakeRunnable);
            mFrontExitTakeHandler.postDelayed(mFrontExitTakeRunnable, TAKETIME);
        }
    }

    private PictureCallback frontJpegCallback = new PictureCallback() {

        public void onPictureTaken(byte[] _data, Camera _camera) {
            Log.i("caoshaowei", " ---onPictureTaken--- " + count);
            Parameters ps = mCamera.getParameters();
            if (ps.getPictureFormat() == PixelFormat.JPEG) {
                //String path = save(_data);
                //Log.d(TAG, "onPictureTaken path " + path);
    ...
    private void setFrontMotorCount() {
        elapsetime_tv.setText(
                getResources().getString(R.string.tltle_front_motor_text)
                        + count + getResources().getString(R.string.text_count_str));
    }

    private void putFrontOpenCount() {
        int sumCount = 0;
        String countStr = app.getSharePref().getValue("open_front_motor_count");
        if (countStr != null && !countStr.isEmpty()) {
            sumCount = Integer.parseInt(countStr);
        }
        sumCount++;
        app.getSharePref().putValue("open_front_motor_count", String.valueOf(sumCount));
    }

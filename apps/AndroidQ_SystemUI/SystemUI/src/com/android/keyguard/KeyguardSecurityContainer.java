...
public class KeyguardSecurityContainer extends FrameLayout implements KeyguardSecurityView {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (FaceUnlockUtil.isFaceUnlockSupport()) {
                    FaceUnlockUtil.getInstance().setOnCenterIconPosition(event.getRawX(), event.getRawY());
                    Log.d(TAG, "onTouchEvent: "+FaceUnlockUtil.getInstance().isIsBouncerShow());
                    //处理上划解锁界面的图标点击事件
                    if(FaceUnlockUtil.getInstance().isIsBouncerShow()&&FaceUnlockUtil.getInstance().startFaceUnlockByRetry(FaceUnlockUtil.FACE_UNLOCK_RETRY_IN_PASSWORD)) {
                        Log.d(TAG, "onTouchEvent  startFaceUnlockByRetry=:");
                    }
                }
    ...
    public int getLayoutIdFor(SecurityMode securityMode) {
        switch (securityMode) {
            ...
            case PIN:
                //modify for EJQQQ-79 by liyuchong 20191123 begin
                //return R.layout.keyguard_pin_view;
                return SystemUIApplication.useDgCustomUI? R.layout.keyguard_pin_view_dg :  R.layout.keyguard_pin_view;
                //modify for EJQQQ-79 by liyuchong 20191123 end

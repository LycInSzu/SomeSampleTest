    ...
    private void updateNextAlarm() {
        synchronized (this) {
            if (withinNHoursLocked(mNextAlarmInfo, ALARM_VISIBILITY_HOURS)) {
                //modify for EJQQ-1552 by liyuchong 20191112 begin
                /*String pattern = android.text.format.DateFormat.is24HourFormat(getContext(),
                        ActivityManager.getCurrentUser()) ? "HH:mm" : "h:mm";*/
                String patternOf12Hour ="h:mm a";
                if (!getContext().getResources().getBoolean(R.bool.show_am_pm_in_keyguardalarmarea)){
                    patternOf12Hour="h:mm";
                }
                String pattern = android.text.format.DateFormat.is24HourFormat(getContext(),
                        ActivityManager.getCurrentUser()) ? "HH:mm" : patternOf12Hour;
                //modify for EJQQ-1552 by liyuchong 20191112 end
...
//add by wuzhi.peng for 	YJSQ-304 start
       java.text.DateFormat shortDateFormat =  android.text.format.DateFormat.getDateFormat(getContext());
        final  java.util.Calendar now = java.util.Calendar.getInstance();
        return shortDateFormat.format(now.getTime()).toString().toUpperCase();
     //   return mDateFormat.format(mCurrentTime);
//add by wuzhi.peng for 	YJSQ-304 end
    }

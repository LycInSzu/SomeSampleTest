...
//        mQSFooter = findViewById(R.id.qs_footer);
        ...
        if (SystemUIApplication.useDgCustomUI||SystemUIApplication.useDkCustomUI) {
            ...
            removeView(findViewById(R.id.qs_footer_blu));
            mQSFooter = findViewById(R.id.qs_footer_dg);
        }else if (SystemUIApplication.useBluCustomUI){
            removeView(findViewById(R.id.qs_footer));
            removeView(findViewById(R.id.qs_footer_dg));
            mQSFooter = findViewById(R.id.qs_footer_blu);
        }else {
            removeView(findViewById(R.id.qs_footer_dg));
            removeView(findViewById(R.id.qs_footer_blu));
            mQSFooter = findViewById(R.id.qs_footer);
...
//        mQSFooter.setTranslationY(height - mQSFooter.getHeight());
        if (SystemUIApplication.useDgCustomUI){
            mQSFooter.setTranslationY(mContext.getResources().getDimensionPixelSize(com.android.internal.R.dimen.quick_qs_offset_height));
        }else {
            mQSFooter.setTranslationY(height - mQSFooter.getHeight());
        }
    ...
    private void setBackgroundGradientVisibility(Configuration newConfig) {
        ...
        if (newConfig.orientation == ORIENTATION_LANDSCAPE) {
            mBackgroundGradient.setVisibility(View.INVISIBLE);
            if (SystemUIApplication.useDgCustomUI){
                ViewGroup.MarginLayoutParams p = (ViewGroup.MarginLayoutParams) mStatusBarBackground.getLayoutParams();
                p.setMargins(mSideMargins, 0, mSideMargins, 0);
                mStatusBarBackground.requestLayout();
            }else {
                mStatusBarBackground.setVisibility(View.INVISIBLE);
            }
        } else {

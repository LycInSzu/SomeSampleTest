    ...
    public QSPanel(@Named(VIEW_CONTEXT) Context context, AttributeSet attrs,
       ...
       /* mBrightnessView = LayoutInflater.from(mContext).inflate(
        ...
        if (SystemUIApplication.useDgCustomUI){
            ...
            mBrightnessView = LayoutInflater.from(mContext).inflate(
                    R.layout.quick_settings_brightness_dialog, this, false);
            ((SeekBar)mBrightnessView.findViewById(R.id.slider)).setThumb(mContext.getResources().getDrawable(R.drawable.ic_brightness_thumb_dg));
            //add for TEJWQE-424 by liyuchong 20200402 begin
            ((ViewGroup) mBrightnessView).removeView(mBrightnessView.findViewById(R.id.settings_icon));
            addView(mBrightnessView);
            ...
            mFooter = new QSSecurityFooter(this, context);
            addView(mFooter.getView());
            findViewById(R.id.settings_icon).setOnClickListener(this::startSettingsActivity);
...
//                findViewById(R.id.brightness_slider));
    ...
    //add for TEWSL-148 by liyuchong 20191021 begin
    static final Intent SETTINGS_INTENT = new Intent(android.provider.Settings.ACTION_SETTINGS);
    private void startSettingsActivity(View v){
        Dependency.get(ActivityStarter.class).startActivity(SETTINGS_INTENT, true /* dismissShade */);
    }
    //add for TEWSL-148 by liyuchong 20191021 end

    protected void addDivider() {
        mDivider = LayoutInflater.from(mContext).inflate(R.layout.qs_divider, this, false);
        mDivider.setBackgroundColor(Utils.applyAlpha(mDivider.getAlpha(),
                getColorForState(mContext, Tile.STATE_ACTIVE)));
        //add for EJQQQ-80 by liyuchong 20191205 begin
        if (SystemUIApplication.useDgCustomUI) {
            mDivider.setBackgroundColor(Utils.applyAlpha(mDivider.getAlpha(),
                    mContext.getResources().getColor(R.color.dg_qs_expaned_background_color)));
        }
        //add for EJQQQ-80 by liyuchong 20191205 end
        addView(mDivider);
    }
    ...
    public void updateResources() {
        ...
        if (SystemUIApplication.useDgCustomUI){
            ...
            setPadding(0, res.getDimensionPixelSize(R.dimen.qs_tile_margin_top_bottom), 0, res.getDimensionPixelSize(R.dimen.qs_panel_padding_bottom));

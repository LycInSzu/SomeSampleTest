...
public class CustomTile extends QSTileImpl<State> implements TileChangeListener {
    ...
    private void updateDefaultTileAndIcon() {
        try {
            ...
            //add for EJQQQ-80 by liyuchong 20191210 begin
            android.util.Log.i("wangjian","updateDefaultTileAndIcon mComponent = " + mComponent);
            if (SystemUIApplication.useDgCustomUI) {
                if(mComponent.getClassName().contains("GrayscaleTileService")) {
                    mDefaultIcon = android.graphics.drawable.Icon
                            .createWithResource(mContext, mTile.getState() == Tile.STATE_ACTIVE ?
                                    R.drawable.ic_gray_style_on : R.drawable.ic_gray_style_off);
                    updateIcon = true;
                }else  if (mComponent.getClassName().contains("StorageAssistantTileService")) {
                    mDefaultIcon = android.graphics.drawable.Icon
                            .createWithResource(mContext, R.drawable.ic_storage_on);
                    updateIcon = true;
                }
                ...
                if(mComponent.getClassName().contains("GrayscaleTileService")) {
                    ...
                    mDefaultIcon = android.graphics.drawable.Icon
                            .createWithResource(mContext, R.drawable.ic_storage_on);
                    updateIcon = true;
                } else if (mComponent.getClassName().contains("FocusModeTileService")) {
                    mDefaultIcon = android.graphics.drawable.Icon
                            .createWithResource(mContext, mTile.getState() == Tile.STATE_ACTIVE ?
                                    R.drawable.ic_focus_mode_on : R.drawable.ic_focus_mode_off);
                    updateIcon = true;
                }
            }
            //add for EWSWQ-275 by liyuchong 20200224 end
            //add for EJQQQ-80 by liyuchong 20191210 end
            if (updateIcon) {
    ...
    public void updateState(Tile tile) {
        //modify for EJQQQ-80 by liyuchong 20191210 begin
//        mTile.setIcon(tile.getIcon());
        android.util.Log.i("wangjian","updateState mComponent = " + mComponent);
        if (SystemUIApplication.useDgCustomUI) {
            if(mComponent.getClassName().contains("GrayscaleTileService")) {
        ...
        //modify for EJQQQ-80 by liyuchong 20191210 end
    ...
    protected void handleUpdateState(State state, Object arg) {
        ...
        android.util.Log.i("wangjian","handleUpdateState mComponent = " + mComponent);
        //add for EJQQQ-80 by liyuchong 20191210 begin
        if (SystemUIApplication.useDgCustomUI) {
            if (mComponent.getClassName().contains("GrayscaleTileService")) {
                state.icon = ResourceIcon.get( mTile.getState() == Tile.STATE_ACTIVE ?
                        R.drawable.ic_gray_style_on : R.drawable.ic_gray_style_off);
                drawable = state.icon.getDrawable(mContext);
            }else if (mComponent.getClassName().contains("StorageAssistantTileService")) {
                state.icon = ResourceIcon.get(R.drawable.ic_storage_on);
                drawable = state.icon.getDrawable(mContext);
            }
            //add for EWSWQ-275 by liyuchong 20200224 begin
        }else if (SystemUIApplication.useDkCustomUI){

...
public class CellularTile extends QSTileImpl<SignalState> {
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        if (SystemUIApplication.useDgCustomUI||SystemUIApplication.useDkCustomUI) {
            if (cb.noSim) {
                state.icon = ResourceIcon.get(R.drawable.ic_no_sim);
            } else {
                state.icon = ResourceIcon.get(state.state == Tile.STATE_ACTIVE ? R.drawable.ic_swap_vert_on : R.drawable.ic_swap_vert_off);
            }
        }
    ...
    private CharSequence getMobileDataContentName(CallbackInfo cb) {
		...
		//add network settings xudong.zhang 20191204(start)
		if(mContext.getResources().getBoolean(com.android.internal.R.bool.config_use_blu_network_settings) && 
				("LTE".equals(cb.dataContentDescription) || "4G".equals(cb.dataContentDescription))){
			return "4G LTE";
		}
		//add network settings xudong.zhang 20191204(end)

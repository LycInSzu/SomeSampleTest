    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        if (SystemUIApplication.useDgCustomUI||SystemUIApplication.useDkCustomUI) {
            if (isTransient) {
                ...
                state.icon = ResourceIcon.get(state.state == Tile.STATE_INACTIVE ? R.drawable.ic_wifi_off : R.drawable.ic_wifi_on);
            }
        }else if (SystemUIApplication.useBluCustomUI) {
            if (isTransient) {
                state.icon = ResourceIcon.get(R.drawable.ic_signal_wifi_transient_animation);
            } else if (!state.value) {
                state.icon = ResourceIcon.get(R.drawable.ic_wifi_on_blu);
            } else if (wifiConnected) {
                state.icon = ResourceIcon.get(R.drawable.ic_wifi_on_blu);
            } else if (wifiNotConnected) {
                state.icon = ResourceIcon.get(R.drawable.ic_qs_wifi_disconnected);

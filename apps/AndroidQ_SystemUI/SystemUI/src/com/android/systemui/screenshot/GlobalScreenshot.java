...
//add Bug_id:XWLYS-55 WTK_FUNNY_SCREEN_SHOT chenkai start
import com.wtk.screenshot.util.ShotUtil;
import android.os.SystemProperties;
//add Bug_id:XWLYS-55 WTK_FUNNY_SCREEN_SHOT chenkai end
//add by wuzhi.peng for liuhai swtich start
import android.provider.Settings;
//add by wuzhi.peng for liuhai switch end
...
class SaveImageInBackgroundTask extends AsyncTask<Void, Void, Void> {
     ...
     * in bitmap.
     */
    //modify for TEWBW-989 by liyuchong 20200305 begin
//    private Bitmap generateAdjustedHwBitmap(Bitmap bitmap, int width, int height, Matrix matrix,
//                                                  Paint paint, int color) {
    public static Bitmap generateAdjustedHwBitmap(Bitmap bitmap, int width, int height, Matrix matrix,
            Paint paint, int color) {
    //modify for TEWBW-989 by liyuchong 20200305 end
...
// modified by zhiheng.huang on 2018/3/9 start >>>>>>>>>>>>>>>>
//class GlobalScreenshot {
public class GlobalScreenshot {
    // modified by zhiheng.huang on 2018/3/9 end <<<<<<<<<<<<<<<<
    ...
    private static final String ACTION_LS_RUNNING_START = "com.cydroid.action.LS_RUNNING_START";
    private static final String ACTION_LS_RUNNING_END = "com.cydroid.action.LS_RUNNING_END";

    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if(action.equals(ACTION_LS_RUNNING_START)){
                NewLongScreenUtil.mLSRunning = true;
                NewLongScreenUtil.postDelayStopLimit();
            }else if(action.equals(ACTION_LS_RUNNING_END)){
                NewLongScreenUtil.mLSRunning = false;
                NewLongScreenUtil.removeCallbacks();
            }
        }
    };
    ...
    //add by wuzhi.peng for YJLL-374 start
    private boolean mIsSupportLiuhai;
    private int mStatusBarHeight;
    //add by wuzhi.peng for YJLL-374 end
    ...
    public GlobalScreenshot(Context context) {
        ...
        //add by wuzhi.peng for YJLL-374 start
        mIsSupportLiuhai = r.getBoolean(com.android.internal.R.bool.config_fillMainBuiltInDisplayCutout);
        mStatusBarHeight = r.getDimensionPixelSize(com.android.internal.R.dimen.status_bar_height);
        //add by wuzhi.peng for YJLL-374 end
        ...
        //add for TEWSL-399 by liyuchong 20200109 begin
        registerLSBReceiver(context);
        //add for TEWSL-399 by liyuchong 20200109 end
        //add for TEWBW-989 by liyuchong 20200305 begin
        sendLSSNForDg(mContext,null);
        //add for TEWBW-989 by liyuchong 20200305 end
    }

    //add for TEWSL-399 by liyuchong 20200109 begin
    private void registerLSBReceiver(Context context){
        IntentFilter filter = new IntentFilter();
        filter.addAction(ACTION_LS_RUNNING_START);
        filter.addAction(ACTION_LS_RUNNING_END);
        context.getApplicationContext().registerReceiverAsUser(mReceiver,android.os.UserHandle.ALL,filter,null,null);
    }
    //add for TEWSL-399 by liyuchong 20200109 end
    ...
    // add by zhiheng.huang on 2018/3/12 start >>>>>>>>>>>>>>>>
    public Bitmap takeWtkScreenshot(Runnable finisher, boolean statusBarVisible, boolean navBarVisible) {
        takeScreenshot((finisher != null ? finisher :new Runnable() {
            @Override
            public void run() {
            }
    ...
    /*Save*/
    public void saveBitMap(Bitmap bitmap,Runnable finishRun){
        if (bitmap == null || bitmap.isRecycled()){
            return;
        }
        Bitmap newBitmap = bitmap.copy(bitmap.getConfig(), false);
        mScreenBitmap = newBitmap;
        startAnimation((finishRun != null ? finishRun :new Runnable() {
                    @Override
                    public void run() {
                    }
                }), mDisplayMetrics.widthPixels,
                mDisplayMetrics.heightPixels, true, true);
    }
    // add by zhiheng.huang on 2018/3/12 end <<<<<<<<<<<<<<<<
    ...
    private void takeScreenshot(Runnable finisher, boolean statusBarVisible, boolean navBarVisible,
            Rect crop) {
        //add for TEWSL-399 by liyuchong 20200109 begin
        if(NewLongScreenUtil.mLSRunning){
            android.util.Log.d("GlobalScreenshot", "takeScreenshot() NewLongScreenUtil.mLSRunning = " + NewLongScreenUtil.mLSRunning);
            finisher.run();
            return;
        }
        //add for TEWSL-399 by liyuchong 20200109 end
        ...
        //add by wuzhi.peng for YJLL-374 start
        if (mIsSupportLiuhai) {
            mScreenBitmap = CropBitmapForLiuhai(mScreenBitmap,mDisplay.getRotation());
        }
        //add by wuzhi.peng for YJLL-374 end
        // Optimizations
        ...
        // modified by zhiheng.huang on 2018/3/9 start >>>>>>>>>>>>>>>>
        boolean showSuperScreenshot = mContext.getResources().getBoolean(R.bool.show_super_screenshot);
    ...
    //add by wuzhi.peng for YJLL-374 start
    private Bitmap CropBitmapForLiuhai(Bitmap bitmap,int orientation){
        ...
        switch (orientation) {
            ...
            case Surface.ROTATION_0 :
                ...
                Settings.System.SWIPE_LIUHAI_STATUS, 0)==1){
                bitmap = Bitmap.createBitmap(bitmap, 0, mStatusBarHeight,bitMapWidth, bitmap.getHeight()-mStatusBarHeight);
            }
                 break;

            case Surface.ROTATION_90 :
                bitmap = Bitmap.createBitmap(bitmap, mStatusBarHeight, 0, bitmap.getWidth()-mStatusBarHeight, bitMapHeight);
                break;

            case Surface.ROTATION_270 :
                bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth()-mStatusBarHeight, bitMapHeight);
                break;
    ...
    //add for TEWBW-989 by liyuchong 20200305 begin
    private static void sendLSSNForDg(Context context, Uri uri){
        if (uri==null){
            return;
        }
        ...
        //Create the large notification icon
        int imageWidth = photoBmp.getWidth();
        int imageHeight = photoBmp.getHeight();
        ...
        if (panelWidth <= 0) {
            // includes notification_panel_width==match_parent (-1)
            panelWidth = r.getDisplayMetrics().widthPixels;
        }
        ...
        Matrix matrix = new Matrix();
        int overlayColor = 0x40FFFFFF;
        matrix.setTranslate((previewWidth - imageWidth) / 2, (previewHeight - imageHeight) / 2);
        Bitmap picture = SaveImageInBackgroundTask.generateAdjustedHwBitmap(photoBmp, previewWidth, previewHeight, matrix,
                paint, overlayColor);

        // Note, we can't use the preview for the small icon, since it is non-square
        float scale = (float) iconSize / Math.min(imageWidth, imageHeight);
        matrix.setScale(scale, scale);
        matrix.postTranslate((iconSize - (scale * imageWidth)) / 2,
                (iconSize - (scale * imageHeight)) / 2);
        Bitmap icon = SaveImageInBackgroundTask.generateAdjustedHwBitmap(photoBmp, iconSize, iconSize, matrix, paint,
                overlayColor);
        // Setup the notification
        final BigPictureStyle mNotificationStyle = new Notification.BigPictureStyle()
                .bigPicture(picture.createAshmemBitmap());

        final long now = System.currentTimeMillis();
        final Notification.Builder notificationBuilder, publicNotificationBuilder;
        final String SCREENSHOT_SHARE_SUBJECT_TEMPLATE = "Screenshot (%s)";
        ...
        publicNotificationBuilder=
                ...
                .setColor(context.getColor(
                        com.android.internal.R.color.system_notification_accent_color));
        SystemUI.overrideNotificationAppName(context, publicNotificationBuilder, true);
        ...
        notificationBuilder = new Notification.Builder(context,
                ...
                .setFlag(Notification.FLAG_NO_CLEAR, false)
                .setPriority(2);
        SystemUI.overrideNotificationAppName(context, notificationBuilder, true);

        notificationBuilder.setLargeIcon(icon.createAshmemBitmap());
        // But we still don't set it for the expanded view, allowing the smallIcon to show here.
        ...
        // Create a share intent, this will always go through the chooser activity first which
        // should not trigger auto-enter PiP
        String subjectDate = DateFormat.getDateTimeInstance().format(new Date(now));
        String subject = String.format(SCREENSHOT_SHARE_SUBJECT_TEMPLATE, subjectDate);
        Intent sharingIntent = new Intent(Intent.ACTION_SEND);
        sharingIntent.setType("image/png");
        sharingIntent.putExtra(Intent.EXTRA_STREAM, uri);
        ...
        notificationManager.notify(SystemMessage.NOTE_GLOBAL_SCREENSHOT,

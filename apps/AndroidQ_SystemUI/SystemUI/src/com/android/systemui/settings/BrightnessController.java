...
public class BrightnessController implements ToggleSlider.Listener {
    ...
    //add for EJWJ-1056 by liyuchong 20191012 begin
    private static boolean SHOW_AUTOMATIC_ICON = false;
    public static boolean isInAutoMode=false;
    ...
    //Add by wangjian for brightness EWWQ-405 20190322 start
    private boolean useLinearBrightnessLevel = false;
    //Add by wangjian for brightness EWWQ-405 20190322 end
    ...
    //modify for EJWJ-1056 by liyuchong 20191012 begin
//    public BrightnessController(Context context, ToggleSlider control) {
    public BrightnessController(Context context, ImageView icon, ToggleSlider control) {
        mIcon = icon;
    //modify for EJWJ-1056 by liyuchong 20191012 end
        mContext = context;
        //add for EJWJ-1056 by liyuchong 20191012 begin
        SHOW_AUTOMATIC_ICON = mContext.getResources().getBoolean(R.bool.show_auto_brightness);
        if (mIcon != null && SHOW_AUTOMATIC_ICON){
            mIcon.setVisibility(View.VISIBLE);
            mIcon.setOnClickListener(this::onClickIcon);
        }else{
            mIcon.setVisibility(View.GONE);
        }
        //add for EJWJ-1056 by liyuchong 20191012 end
        //Add by wangjian for brightness EWWQ-405 20190322 start
        useLinearBrightnessLevel = mContext.getResources().getBoolean(com.android.internal.R.bool.config_use_linear_brightness_level);
        //Add by wangjian for brightness EWWQ-405 20190322 end
        mControl = control;
    ...
    public void onChanged(ToggleSlider toggleSlider, boolean tracking, boolean automatic,
        ...
        //modify by wangjian for brightness EWWQ-405 20190322 start
        //final int val = convertGammaToLinear(value, min, max);
        int valTemp = 0;
        if (useLinearBrightnessLevel) {
            float scale = (value * 1.0f / GAMMA_SPACE_MAX);
            //modify by wangjian for EWSY-1169 20190907 start
            //valTemp =  (int) (scale * 255);
            valTemp =  (int) (scale * 245) + 10;
            //modify by wangjian for EWSY-1169 20190907 end
        } else {
            valTemp = convertGammaToLinear(value, min, max);
        }
        final int val = valTemp;
        //modify by wangjian for brightness EWWQ-405 20190322 end
    ...
    private void updateIcon(boolean automatic) {
        if (mIcon != null) {
            if (automatic && SHOW_AUTOMATIC_ICON){
                isInAutoMode=true;
            }else {
                isInAutoMode=false;
            }
    ...
    private void updateSlider(int val, boolean inVrMode) {
        ...
        if (useLinearBrightnessLevel) {
            //modify by wangjian for EWSY-1169 20190907 start
            //float scale = (val * 1.0f / 255);
            float scale = ((val - 10) * 1.0f / 245);
            //modify by wangjian for EWSY-1169 20190907 end
            val =  (int) (scale * GAMMA_SPACE_MAX);
            animateSliderTo(val);
            return;
        }
        //modify by wangjian for brightness EWWQ-405 20190322 end
    ...
    //add for EJWJ-1056 by liyuchong 20191012 begin
	private void onClickIcon(View v){
		if(v == mIcon){
			//make sure mAutomaticAvailable is true!
            int mode =mAutomatic?Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL:Settings.System.SCREEN_BRIGHTNESS_MODE_AUTOMATIC;
            mBackgroundHandler.removeCallbacks(mTask);
            mTask.setBrightnessMode(mode);
            mBackgroundHandler.post(mTask);
		}
	}

	final SetBrightnessModeTask mTask = new SetBrightnessModeTask();
	private  class SetBrightnessModeTask implements Runnable{

...
public class StatusBarMobileView extends FrameLayout implements DarkReceiver,
    ...
    //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
    static final int ICON_STAT_SYS_SIGNAL_BG = R.drawable.stat_sys_signal_bg;//actual signal level four
    static final int ICON_STAT_SYS_SIGNAL_NULL = R.drawable.stat_sys_signal_null;
    static final int ICON_STAT_SYS_SIGNAL_LEVEL_ZERO = R.drawable.stat_sys_signal_level_0;//can any level
    private TextView mSlotIndicator;
    private int mLastMobileDarkIconId = ICON_STAT_SYS_SIGNAL_NULL;
    ...
    //add for EJQQ-1115 by liyuchong 20191030 begin
    private View mMsMobileDataContainer;
    //add for EJQQ-1115 by liyuchong 20191030 end
    ...
    /// @ }
    //add by wangjian for liuhai start
    private boolean shouldShowMobileIcon = false;
    //add by wangjian for liuhai end
    ...
    public static StatusBarMobileView fromContext(Context context, String slot) {
        ...
        if (SystemUIApplication.useDgCustomUI) {
            v = (StatusBarMobileView)
                    inflater.inflate(R.layout.status_bar_mobile_signal_group_dg, null);
        }
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        v.setSlot(slot);
        v.init();
    ...
    private void init() {
        ...
        if (SystemUIApplication.useDgCustomUI) {
            ...
            mMobileDark = findViewById(R.id.mobile_signal_dark);
         }
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        ...
        //add for EJQQ-1115 by liyuchong 20191030 begin
        mMsMobileDataContainer = findViewById(R.id.ms_mobile_data_container);
        //add for EJQQ-1115 by liyuchong 20191030 end
        ...
        /// @}
        //modify for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
        ...
        if (!SystemUIApplication.useDgCustomUI) {
            ...
            setIconForView(mMobileDark, ICON_STAT_SYS_SIGNAL_NULL);
        }
        mSlotIndicator = (TextView) findViewById(R.id.mobile_slot_indicator_text);
        //modify for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        initDotView();
    ...
    private void initViewState() {
        ...
        if (SystemUIApplication.useDgCustomUI) {
            updateSignalStrengthIcon(mState);
        } else {
            mMobileDrawable.setLevel(mState.strengthId);
        }
        //modify for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        ...
        if (mState.typeId > 0) {
            ...
            mMsMobileDataContainer.setVisibility(View.VISIBLE);
            mMobileType.setVisibility(View.VISIBLE);
        } else {
            mMsMobileDataContainer.setVisibility(View.GONE);
...
//                ? View.VISIBLE : View.GONE);
        mIn.setVisibility(mState.activityIn ? View.VISIBLE : View.INVISIBLE);
        mOut.setVisibility(mState.activityOut ? View.VISIBLE : View.INVISIBLE);
        //modify for EJQQ-1115 by liyuchong 20191030 end
        /// M: Add for [Network Type and volte on Statusbar] @{
     ...
     //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
    private void updateSignalStrengthIcon(MobileIconState state) {
        updateMobileDark(state.strengthId);
        setIconForView(mMobile, state.strengthId);
        boolean show = (state.strengthId != ICON_STAT_SYS_SIGNAL_NULL && state.strengthId != ICON_STAT_SYS_SIGNAL_LEVEL_ZERO);
        mMobile.setVisibility(show ? View.VISIBLE : View.INVISIBLE);
    }

    private void updateMobileDark(int mobileIconId) {
        int mobileDarkIconId = (mobileIconId == ICON_STAT_SYS_SIGNAL_NULL ? ICON_STAT_SYS_SIGNAL_NULL : ICON_STAT_SYS_SIGNAL_BG);
        boolean change = mLastMobileDarkIconId != mobileDarkIconId;
        mLastMobileDarkIconId = mobileDarkIconId;
        if (change) {
            setIconForView(mMobileDark, mobileDarkIconId);
        }
    }

    private void setIconForView(ImageView v, int resId) {
        try {
            //Thread.dumpStack();
            Drawable icon = v.getContext().getDrawable(resId);
            if (icon != null) {
...
//        }
        ...
        mState = state;
        //delete by wangjian for TEWBW-433 20191226 start
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
        //if (SystemUIApplication.useDgCustomUI) {
        //    updateSlotIndicatorVisibility();
        //}
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        //delete by wangjian for TEWBW-433 20191226 end
    ...
    public void onDarkChanged(Rect area, float darkIntensity, int tint) {
        ...
        int tempColor = getTint(area, this, tint);
        if (SystemUIApplication.useDgCustomUI) {
            tempColor = isInArea(area, this) ? getTint(area, this, tint)
                    : getColorForDarkIntensity(0, Utils.getColorAttrDefaultColor(mLightContext, R.attr.fillColor),
                    Utils.getColorAttrDefaultColor(mDarkContext, R.attr.fillColor));
        }else {
            if (!isInArea(area, this)) {
                return;
            }
        ...
        ColorStateList color = ColorStateList.valueOf(tempColor);
        //modify for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        ...
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
        if (SystemUIApplication.useDgCustomUI) {
            mMobileDark.setImageTintList(color);
            mMobileDark.setAlpha(0.5f);
        }
        mSlotIndicator.setTextColor(color);
        //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
        /// @}
    }
    ...
    public void setStaticDrawableColor(int color) {
        ...
        if (SystemUIApplication.useDgCustomUI) {
            mMobileDark.setImageTintList(list);
            mMobileDark.setAlpha(0.5f);
        } else {
            mMobileDrawable.setTintList(
                    ColorStateList.valueOf(mDualToneHandler.getSingleColor(intensity)));
        }
        mSlotIndicator.setTextColor(list);
        //modify for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 end
    ...
    //add by wangjian for liuhai start
    public void setShouldShowMobileIcon(boolean show) {
        shouldShowMobileIcon = show;
        setCustomizeViewProperty();
    }

    public boolean shouldShowMobileIcon() {
        if (StatusBar.isLiuhaiSupport) {
            return shouldShowMobileIcon;
        }
        return true;
    }
    //add by wangjian for liuhai end
    ...
    private void setVolteIcon(int volteIcon) {
        //add by wangjian for liuhai start
		//modify YJSQ-2551 xudong.zhang 20190225(start)
        //if (!shouldShowMobileIcon()) {
        if (!shouldShowMobileIcon() && !mContext.getResources().getBoolean(R.bool.config_show_volte_type)) {
		//modify YJSQ-2551 xudong.zhang 20190225(end)
    ...
    private void setNetworkIcon(int networkIcon) {
        ...
        //add by wangjian for liuhai start
		//modify AQJB-714 xudong.zhang 20190221(start)
        //if (!shouldShowMobileIcon()) {
        if (!shouldShowMobileIcon() && !mContext.getResources().getBoolean(R.bool.config_show_network_type)) {
		//modify AQJB-714 xudong.zhang 20190221(end)
...
//            mNetworkType.setVisibility(View.VISIBLE);
            if (mMobileType.getVisibility()==View.VISIBLE){
                mNetworkType.setVisibility(View.GONE);
            }else {
                mNetworkType.setVisibility(View.VISIBLE);
            }
...
//            mMobileType.setVisibility(View.GONE);
    ...
    //add for EJQQQ-94 modify mobile signal icon by liyuchong 20191122 begin
    private int getSlotIndicatorIndex() {
        return SIMHelper.getSlotCount() < 2 ? 0 : mState.phoneId == 0 ? 1 : mState.phoneId == 1 ? 2 : 0;
    }

    private void updateSlotIndicatorVisibility() {
        final int slotIndicatorIconId = getSlotIndicatorIndex();
        boolean shouldHideSimIndicator = slotIndicatorIconId == 0;
        if (shouldHideSimIndicator) {
            mSlotIndicator.setVisibility(View.GONE);
        } else {
            mSlotIndicator.setText(String.valueOf(slotIndicatorIconId));

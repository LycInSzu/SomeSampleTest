...
public class NotificationEntryManager implements
    ...
    public void onAsyncInflationFinished(NotificationEntry entry,
        ...
        if (!entry.isRowRemoved()) {
            ...
            if (isNew) {
				...
				//add EJSLY-224 xudong.zhang 20190823(start)
				PowerManager mPowerManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
				if (!mPowerManager.isScreenOn() && entry.key != null && 
						entry.key.contains("com.google.android.apps.messaging") &&
						mContext.getResources().getBoolean(R.bool.config_wakeup_screen_when_receiving_msg)){
					PowerManager.WakeLock mNotificationWakeLock = mPowerManager.newWakeLock(PowerManager.FULL_WAKE_LOCK
							| PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "MMS_wake_lock");
					mNotificationWakeLock.acquire(5000);
				}
				//add EJSLY-224 xudong.zhang 20190823(end)	
    ...
    public void addNotification(StatusBarNotification notification,
        ...
        //add for notification hide start
        if(shouldHideNotification(notification)){
            return;
        }
        //add for notification hide end
        try {
    ...
    //add for notification hide start
    private boolean shouldHideNotification(StatusBarNotification notification){
        String pkName = notification.getPackageName();
        if (pkName != null && (pkName.equals("com.mediatek.camera")
                || (pkName.equals("com.android.nfc")&&notification.getNotification()!=null
                    &&"NFC_channel".equals(notification.getNotification().getChannel()))
                || (pkName.equals("com.android.music")&&notification.getNotification()!=null
                    &&("com.android.music".equals(notification.getNotification().getChannel()) || notification.getNotification().contentView == null))
                || (pkName.equals("android")&&notification.getNotification()!=null &&"FOREGROUND_SERVICE".equals(notification.getNotification().getChannel()))
                || (pkName.equals("com.adups.fota")&&notification.getId() == 1)
                || pkName.equals("com.cydroid.softmanager")
                || pkName.equals("com.android.camera"))) {
            return true;
        }
        return false;
    }
//add for notification hide end
    ...
    private void updateNotificationInternal(StatusBarNotification notification,
		...
		//add EJSLY-224 xudong.zhang 20190823(start)
		PowerManager mPowerManager = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
		if (!mPowerManager.isScreenOn() && notification.getPackageName() != null && 
				notification.getPackageName().contains("com.google.android.apps.messaging") &&
				mContext.getResources().getBoolean(R.bool.config_wakeup_screen_when_receiving_msg)){
			PowerManager.WakeLock mNotificationWakeLock = mPowerManager.newWakeLock(PowerManager.FULL_WAKE_LOCK
					| PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "MMS_wake_lock");
			mNotificationWakeLock.acquire(5000);

...
import static android.provider.Settings.System.GAME_MODE_BLOCK_CALL;
import static android.provider.Settings.System.GAME_MODE_CANCEL_NOTIFY;
...
public class NotificationInterruptionStateProvider {
    ...
    public boolean shouldHeadsUp(NotificationEntry entry) {
		...
		// add BUG_ID:TEJBB-221 donghao 20200319 start
        if (mContext.getResources().getBoolean(R.bool.call_always_fullscreen)) {
            inUse = inUse && !sbn.getPackageName().equals("com.android.dialer");
        }
        // add BUG_ID:TEJBB-221 donghao 20200319 end
    ...
    public boolean canHeadsUpCommon(NotificationEntry entry) {
        ...
        //add EWSWQ-85 xudong.zhang 20200115(start)
        final boolean inGameMode = Settings.System.getInt(mContext.getContentResolver(), IN_GAME_MODE, 0) == 1;
        final boolean isBlockCall = Settings.System.getInt(mContext.getContentResolver(), GAME_MODE_BLOCK_CALL, 0) == 1;
        final boolean isCancelNotify = Settings.System.getInt(mContext.getContentResolver(), GAME_MODE_CANCEL_NOTIFY, 0) == 1;
        final boolean isPhoneNotify = sbn.getPackageName().equals("com.android.dialer") || sbn.getPackageName().equals("com.google.android.dialer");
        if (inGameMode) {
            if (isPhoneNotify) {
                if (isBlockCall && isCancelNotify) {
                    android.util.Log.i(TAG, "game mode block headsup");
                    return false;
                } else {
                    // if user don`t want block calls and want block notify,
                    // when a call coming, there will enter in-call screen.
                    // so, we will show the notify
                    return true;
                }
            } else if (isCancelNotify) {
                android.util.Log.i(TAG, "game mode block headsup");
                return false;
            }
        }
        //add EWSWQ-85 xudong.zhang 20200115(end)

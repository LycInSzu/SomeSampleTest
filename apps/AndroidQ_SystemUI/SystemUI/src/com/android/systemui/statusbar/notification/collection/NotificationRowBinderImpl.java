...
public class NotificationRowBinderImpl implements NotificationRowBinder {
    ...
    //add for EWSWQ-50 by liyuchong 20200116 begin
    private ExecutorService mDatabaseCachedThreadPoll= Executors.newCachedThreadPool();
    //add for EWSWQ-50 by liyuchong 20200116 end
    ...
    private void updateNotification(
        ...
        //add for EWSWQ-50 by liyuchong 20200116 begin
        if (SystemUIApplication.dkModifyNotiSortLogic&& !SystemUIApplication.isInCtsNotificationTest() &&(!row.getIsNonblockable())) {
            mDatabaseCachedThreadPoll.execute(new UpdateImportanceRunnable(INotificationManager.Stub.asInterface(
                    android.os.ServiceManager.getService(Context.NOTIFICATION_SERVICE)), sbn.getPackageName(), sbn.getUid(),
                    row.getUniqueChannels().size() == 1 ? row.getEntry().channel : null,
                    row.getEntry().channel.getImportance()));
            }
        //add for EWSWQ-50 by liyuchong 20200116 end
        }
    ...
    private class UpdateImportanceRunnable implements Runnable {
        ...
        private int mNewImportance = android.app.NotificationManager.IMPORTANCE_LOW;
        ...
        public void run() {
            try {
                ...
                Cursor cursor = readdb.rawQuery(readsql, new String[] { mPackageName });
                boolean needUpdate = false;
                if (cursor.getCount() <= 0) {
                    if (mCurrentImportance >= android.app.NotificationManager.IMPORTANCE_DEFAULT){
                        needUpdate = true;
                        mNewImportance = android.app.NotificationManager.IMPORTANCE_LOW;
                    }
                }else {
                    while (cursor.moveToNext()) {
                        int importance = cursor.getInt(cursor.getColumnIndex("important"));
                        if (importance == 0 && mCurrentImportance < android.app.NotificationManager.IMPORTANCE_DEFAULT){
                            needUpdate = true;
                            mNewImportance = android.app.NotificationManager.IMPORTANCE_DEFAULT;
                        }else if(importance == 1 && mCurrentImportance >= android.app.NotificationManager.IMPORTANCE_DEFAULT){
                            needUpdate = true;
                            mNewImportance = android.app.NotificationManager.IMPORTANCE_LOW;
                        }
                    }

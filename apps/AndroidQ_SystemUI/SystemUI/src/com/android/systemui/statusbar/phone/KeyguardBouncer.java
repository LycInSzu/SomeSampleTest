...
//    modify for EJSLYQ-66 by liyuchong 20200102 begin
//    protected ViewGroup mRoot;
    protected WTKBouncerRootView mRoot;
//    modify for EJSLYQ-66 by liyuchong 20200102 end
    ...
    private final Runnable mShowRunnable = new Runnable() {
        ...
        public void run() {
            ...
            //FACE_UNLOCK_SUPPORT start
            if (FaceUnlockUtil.isFaceUnlockSupport() && FaceUnlockUtil.getInstance().unlockingAllowed()) {
                FaceUnlockUtil.getInstance().runKeyguardAnimate();
                String message = FaceUnlockUtil.getInstance().getMessage();
                Log.d(TAG, "run: message"+message);
                if (message != null ) {
                    Log.d(TAG, "run: message1"+message);
                    showMessage(message, ColorStateList.valueOf(Color.WHITE));
                }
            }
            //FACE_UNLOCK_SUPPORT end
    ...
    protected void inflateView() {
        ...
        mRoot = (WTKBouncerRootView) LayoutInflater.from(mContext).inflate(R.layout.keyguard_bouncer, null);
        //modify for EJSLYQ-66 by liyuchong 20200102 end

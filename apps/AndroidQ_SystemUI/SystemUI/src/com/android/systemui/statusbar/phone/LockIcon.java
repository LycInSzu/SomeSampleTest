...
public class LockIcon extends KeyguardAffordanceView implements OnUserInfoChangedListener,
    ...
    private static final int STATE_FINGERPRINT = 4;
    ...
    public void setTransientBiometricsError(boolean transientBiometricsError) {
        //modify for EJQQ-1078 by liyuchong 20191101 begin
        //mTransientBiometricsError = transientBiometricsError;
        if (!KeyguardUpdateMonitor.IS_FINGERPRINT_LOCKOUT) {
            mTransientBiometricsError = transientBiometricsError;
        }else {
            mTransientBiometricsError = false;
        }
        //modify for EJQQ-1078 by liyuchong 20191101 end
        update();
    }
...
//            boolean isAnim = iconAnimRes != -1;
            ...
            if (iconRes != mIconRes) {
                ...
                final AnimatedVectorDrawable animation = icon instanceof AnimatedVectorDrawable
                        ...
                        : null;
                //FACE_UNLOCK_SUPPORT start
                if (FaceUnlockUtil.isFaceUnlockSupport() && FaceUnlockUtil.getInstance().setLockIconBg(false)) {
                }else {
                //FACE_UNLOCK_SUPPORT end
            ...
            //FACE_UNLOCK_SUPPORT start
            }else if(FaceUnlockUtil.getInstance().isAllowSetSystemLockIconBg() ){
                Log.d(TAG, "update: state="+state);
                Drawable iconNoFace = mContext.getDrawable(getIconForState(state));
                setImageDrawable(iconNoFace, false);
                //FACE_UNLOCK_SUPPORT end
            } else if(FaceUnlockUtil.isFaceUnlockSupport() && FaceUnlockUtil.getInstance().setLockIconBg(false)) {
                //FACE_UNLOCK_SUPPORT end
                }
            updateDarkTint();
        ...
        //modify by wangjian for EJSLYQ-94 20200107 start
        //boolean invisible = onAodNotPulsingOrDocked || mWakeAndUnlockRunning
        //        || mShowingLaunchAffordance;
        boolean invisible = onAodNotPulsingOrDocked || mWakeAndUnlockRunning
                || mShowingLaunchAffordance || mKeyguardGone;
        //modify by wangjian for EJSLYQ-94 20200107 end
    ...
    private int getState() {
        ...
        //add for EJQQ-1078 by liyuchong 20191101 begin
        boolean isUnlockAllowed=updateMonitor.isUnlockingWithBiometricAllowed();
        //add for EJQQ-1078 by liyuchong 20191101 end
        ...
        //add for EJQQ-1078 by liyuchong 20191101 begin
        }else if (updateMonitor.isFingerprintDetectionRunning() && isUnlockAllowed && !KeyguardUpdateMonitor.IS_FINGERPRINT_LOCKOUT) {
    ...
    public void onBiometricAuthModeChanged(boolean wakeAndUnlock) {
        ...
        update();
        //add for EJQQQ-715 by liyuchong 20200302 begin
        mWakeAndUnlockRunning = false;
        //add for EJQQQ-715 by liyuchong 20200302 end
    }
    ...
    //add by wangjian for EJSLYQ-94 20200107 start
    public void setKeyguardGone(boolean mKeyguardGone) {
        this.mKeyguardGone = mKeyguardGone;
    }
    //add by wangjian for EJSLYQ-94 20200107 end
}

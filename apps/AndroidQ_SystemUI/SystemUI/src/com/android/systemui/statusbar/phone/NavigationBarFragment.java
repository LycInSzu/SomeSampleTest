...
import static android.provider.Settings.System.IN_GAME_MODE;
...
public class NavigationBarFragment extends LifecycleFragment implements Callbacks,
    ...
    boolean onHomeLongClick(View v) {
        //add BUG_ID:YJSQ-1076 wangjian 20181214 (start)
        if (isInCitTest()) {
            return true;
        }
        //add BUG_ID:YJSQ-1076 wangjian 20181214 (end)

        // add BUG_ID:YJSQ-3678 zhangzhanbo 20190425 start
        if (isInGameMode()) {
            Log.i(TAG, "in game mode, ignore this");
            return true;
        }
        // add BUG_ID:YJSQ-3678 zhangzhanbo 20190425 end
    ...
    private void onRecentsClick(View v) {
        ...
        //add BUG_ID:YJSQ-23 liuzhijun 20181106 (start)
        if (isInCitTest()) {
            sendEvent(KeyEvent.KEYCODE_MENU, KeyEvent.ACTION_DOWN);
            sendEvent(KeyEvent.KEYCODE_MENU, KeyEvent.ACTION_UP);
            return;
        }
        //add BUG_ID:YJSQ-23 liuzhijun 20181106 (end)
    ...
    //add BUG_ID:YJSQ-23 liuzhijun 20181106 (start)
    private void sendEvent(int code, int action) {
        long time = SystemClock.uptimeMillis();

        final KeyEvent ev = new KeyEvent(time, time, action, code, 0,
    ...
    private boolean onLongPressBackRecents(View v) {
        //add by liutao for EJSL-767 20190114 start
        if (isInCitTest()) {
            return true;
        }
        //add by liutao for EJSL-767 20190114 end
    ...
    private boolean onLongPressNavigationButtons(View v, @IdRes int btnId1, @IdRes int btnId2) {
        try {
            //add for EJQQ-1855 by liyuchong 20191210 begin
            ...
            //add for EJQQ-1855 by liyuchong 20191210 end
    ...
    public boolean isInCitTest() {
        ...
        if (tasks != null && !tasks.isEmpty()) {
            //modify by jinhui.li YJSQ-3116 start {
            //isInCitTest = "com.wtk.factory".equals(tasks.get(0).topActivity.getPackageName());
            String mTopPkg = tasks.get(0).topActivity.getPackageName();
            String mTopClass = tasks.get(0).topActivity.getClassName();
            isInCitTest = "com.wtk.factory".equals(mTopPkg) || "com.android.settings.fingerprint.FingerprintEnrollFindSensor".equals(mTopClass)
                          || "com.android.settings.fingerprint.FingerprintEnrollEnrolling".equals(mTopClass);
            //modify by jinhui.li YJSQ-3116 end }
        }
    ...
    /**
     * add BUG_ID:YJSQ-3678 zhangzhanbo 20190425
     * @return
     */
    public boolean isInGameMode() {
        ...
        if (tasks != null && !tasks.isEmpty()) {
            ...
            isInGameMode = topActivity.contains("com.wtk.gamemode");
        }
        isInGameMode = /*isInGameMode &&*/
                Settings.System.getInt(getContext().getContentResolver(), IN_GAME_MODE, 0) == 1;

...
//add BUG_ID:YJSQ-23 liuzhijun 20181106 (start)
import android.app.ActivityManager;
//add BUG_ID:YJSQ-23 liuzhijun 20181106 (end)
...
public class NavigationBarView extends FrameLayout implements
    ...
    private boolean mIsSecondStyle = false;
    private boolean mIsNtfPanelContorlEnabled;
    private KeyButtonDrawable mNavibarIcon, mNavibarLandIcon;
    private KeyButtonDrawable mNtfpanelIcon, mNtfpanelIconExpanded;
    ...
    private final OnComputeInternalInsetsListener mOnComputeInternalInsetsListener = info -> {
        ...
        //modify for EJQQ-1855 by liyuchong 20191210 begin
        /*if (!isGesturalMode(mNavBarMode) || mImeVisible) {
            info.setTouchableInsets(InternalInsetsInfo.TOUCHABLE_INSETS_FRAME);
            return;
        }*/
        if (!isGesturalMode(mNavBarMode) || mImeVisible || ActivityManagerWrapper.getInstance().isScreenPinningActive()) {
            info.setTouchableInsets(InternalInsetsInfo.TOUCHABLE_INSETS_FRAME);
            return;
        }
        //modify for EJQQ-1855 by liyuchong 20191210 end
    ...
    public NavigationBarView(Context context, AttributeSet attrs) {
        ...
        // add by wangjian for TEJWQE-414 20200331 start
        mButtonDispatchers.put(R.id.navigationbar, new ButtonDispatcher(R.id.navigationbar));
        mButtonDispatchers.put(R.id.notification_panel_control,new ButtonDispatcher(R.id.notification_panel_control));
        // add by wangjian for TEJWQE-414 20200331 end
    ...
    // add by wangjian for TEJWQE-414 20200331 start
    public ButtonDispatcher getNavigationBarButton() {
        return mButtonDispatchers.get(R.id.navigationbar);
    }

    public ButtonDispatcher getNtfPanelContorlButton() {
        return mButtonDispatchers.get(R.id.notification_panel_control);
    }

    public void setIsSecondStyle(boolean isSecondStyle) {
        mIsSecondStyle = isSecondStyle;
    }
    private boolean isSecondStyle() {
        return mIsSecondStyle;
    }
    public View getNavButtons() {
        return mCurrentView.findViewById(R.id.nav_buttons);
    }

    public View getSecondNavButtons() {
        return mCurrentView.findViewById(R.id.nav_buttons_two);
    }

    public View getCurrentNavButtons() {
        return isSecondStyle() ? getSecondNavButtons() : getNavButtons();
    }

    public void setNtfPanelContorlEnabled(boolean enabled) {
        mIsNtfPanelContorlEnabled = enabled;
    }
    ...
    public void setNtfPanelContorlState() {
        getNtfPanelContorlButton().setVisibility(mIsNtfPanelContorlEnabled ? View.VISIBLE : View.INVISIBLE);
    }

    public void setNavibarArrowState(){
        getNavigationBarButton().setVisibility(mIsNavibarEnabled ? View.VISIBLE : View.INVISIBLE);
    }
    // add by wangjian for TEJWQE-414 20200331 end
    ...
    private void updateIcons(Configuration oldConfig) {
        ...
        // add by wangjian for TEJWQE-414 20200331 start
        mNavibarIcon = getDrawable( R.drawable.ic_sysbar_navigationbar_default);
        mNavibarLandIcon = getDrawable(R.drawable.ic_sysbar_navigationbar_land_default);
        mNtfpanelIcon = getDrawable(R.drawable.ic_sysbar_ntfpanel_control);
        mNtfpanelIconExpanded = getDrawable(R.drawable.ic_sysbar_ntfpanel_control_expanded);
        // add by wangjian for TEJWQE-414 20200331 end
    }
    ...
    //add by wangjian for TEJWQ-262 20200331 start
    public KeyButtonDrawable getRecentsDrawable() {
        KeyButtonDrawable drawable = mRecentIcon = getDrawable(R.drawable.ic_sysbar_recent);
        return drawable;
    }
    //add by wangjian for TEJWQ-262 20200331 end
    ...
    public void updateNavButtonIcons() {
        ...
        // add by wangjian for TEJWQE-414 20200331 start
        KeyButtonDrawable navibarIcon = mIsVertical ? mNavibarLandIcon : mNavibarIcon;
        getNavigationBarButton().setImageDrawable(navibarIcon);
        getNtfPanelContorlButton().setImageDrawable(mNtfpanelIcon);
        // add by wangjian for TEJWQE-414 20200331 end
        ...
        getRecentsButton().setVisibility((disableRecent && !isInCitTest()) ? View.INVISIBLE : View.VISIBLE);
        //add BUG_ID:YJSQ-23 liuzhijun 20181106 (end)
    }

    //add BUG_ID:YJSQ-23 liuzhijun 20181106 (start)
    public boolean isInCitTest() {
        ActivityManager am = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
        boolean isInCitTest = false;
        try {
            isInCitTest = "com.wtk.factory".equals(am.getRunningTasks(1).get(0).topActivity.getPackageName());
        } catch (Exception e) {
        }
    ...
    private void updateCurrentView() {
        ...
        updateLayoutTransitionsEnabled();
        // add by wangjian for TEJWQE-414 20200331 start
        if(HAVE_SMART_NAVIBAR && mOldRotation != mCurrentRotation){
            mOldRotation = mCurrentRotation;
            getNavButtons().setVisibility(isSecondStyle() ? View.GONE : View.VISIBLE);
            getSecondNavButtons().setVisibility(isSecondStyle() ? View.VISIBLE : View.GONE);
        }
        // add by wangjian for TEJWQE-414 20200331 end
    }

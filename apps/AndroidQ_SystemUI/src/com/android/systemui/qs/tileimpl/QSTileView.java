...
public class QSTileView extends QSTileBaseView {
    ...
    protected void createLabel() {
        ...
        if (SystemUIApplication.useDgCustomUI){
            mLabelContainer = (ViewGroup) LayoutInflater.from(getContext())
                    .inflate(R.layout.qs_tile_label_dg, this, false);
        }
    ...
    protected void handleStateChanged(QSTile.State state) {
		...
		//add for EJQQQ-80 by liyuchong 20191204 begin
        if (SystemUIApplication.useDgCustomUI) {
            mLabel.setTextColor(state.state == Tile.STATE_ACTIVE ? getContext().getColor(R.color.dg_systemui_theme_color_accent) : getContext().getColor(R.color.dg_systemui_theme_color_inactive));
            mSecondLine.setTextColor(getContext().getColor(R.color.grey_dg));
        }
		//add for EJQQQ-80 by liyuchong 20191204 end
        //add for EWSWQ-275 by liyuchong 20200220 begin
        if (SystemUIApplication.useDkCustomUI) {
            mLabel.setTextColor(state.state == Tile.STATE_ACTIVE ? getContext().getColor(R.color.dk_systemui_qs_color_accent) : getContext().getColor(R.color.dk_systemui_qs_color_inactive));
            mSecondLine.setTextColor(getContext().getColor(R.color.dk_systemui_qs_color_inactive));
        }

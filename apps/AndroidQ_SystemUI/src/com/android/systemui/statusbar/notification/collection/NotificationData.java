...
public class NotificationData {
    ...
    protected final Comparator<NotificationEntry> mRankingComparator =
        ...
        public int compare(NotificationEntry a, NotificationEntry b) {
            ...
            boolean aDKUsual = a.isHighPriority();
            boolean bDKUsual = b.isHighPriority();
            //add for EWSWQ-50 by liyuchong 20200116 end

            //modify for EWSWQ-50 by liyuchong 20200116 begin
...
//            b.setIsTopBucket(bHeadsUp || bMedia || bSystemMax || b.isHighPriority());
            if (SystemUIApplication.dkModifyNotiSortLogic){
                a.setIsTopBucket(aDKUsual);
                b.setIsTopBucket(bDKUsual);
            }else {
                a.setIsTopBucket(aHeadsUp || aMedia || aSystemMax || a.isHighPriority());
                b.setIsTopBucket(bHeadsUp || bMedia || bSystemMax || b.isHighPriority());
            }
            //modify for EWSWQ-50 by liyuchong 20200116 end

            //add for EWSWQ-50 by liyuchong 20200116 begin
            if (SystemUIApplication.dkModifyNotiSortLogic && aDKUsual) {
                return  -1 ;
            } else if (SystemUIApplication.dkModifyNotiSortLogic && (aDKUsual != bDKUsual)) {
                return aDKUsual ? -1 : 1;
            //add for EWSWQ-50 by liyuchong 20200116 end
...
//                && mTmpRanking.getImportance() >= NotificationManager.IMPORTANCE_LOW;
        return notification.isForegroundService()
                && (SystemUIApplication.dkModifyNotiSortLogic?(mTmpRanking.getImportance() >= NotificationManager.IMPORTANCE_DEFAULT):(mTmpRanking.getImportance() >= NotificationManager.IMPORTANCE_LOW));
        //modify for EWSWQ-50 by liyuchong 20200116 end
    }

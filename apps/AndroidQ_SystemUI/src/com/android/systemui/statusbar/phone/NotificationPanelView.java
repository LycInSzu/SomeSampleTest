...
//add by wuzhi.peng for liuhai switch start
import static android.provider.Settings.System.SWIPE_TOPLEFT_HIDDEN_LIUHAI;
import static android.provider.Settings.System.SWIPE_LIUHAI_STATUS;
...
public class NotificationPanelView extends PanelView implements
    ...
    private boolean mShowInterceptTouchEvent;
    //add by wuzhi.peng for liuhai switch end
    //add by wangjian for EJQQ-2029 20191210 start
    ...
    //add by wangjian for EJQQ-2029 20191210 end
    ...
    public NotificationPanelView(@Named(VIEW_CONTEXT) Context context, AttributeSet attrs,
        ...
        //add by wuzhi.peng for liuhai switch
        mSettingObserver = new SettingObserver(new Handler(context.getMainLooper()));
        mShowInterceptTouchEvent = getLiuhaiInterceptSettingsValue();
        //add by wuzhi.peng for liuhai end
        //add by wangjian for EJQQ-2029 20191210 start
        context.getContentResolver().registerContentObserver(
                Settings.Global.getUriFor(EXTREME_MODE),false,mExtremePowerSaveObserver);
        mExtremeMode = Settings.Global.getInt(mContext.getContentResolver(), EXTREME_MODE, POWER_MODE_NORMAL) == POWER_MODE_EXTREME;
        //add by wangjian for EJQQ-2029 20191210 end
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        //add by wuzhi.peng for liuhai switch start
        mSettingObserver.destroy();
        //add by wuzhi.peng for liuhai switch end
        //add by wangjian for EJQQ-2029 20191210 start
        mContext.getContentResolver().unregisterContentObserver(mExtremePowerSaveObserver);
        mExtremeMode = Settings.Global.getInt(mContext.getContentResolver(), EXTREME_MODE, POWER_MODE_NORMAL) == POWER_MODE_EXTREME;
        //add by wangjian for EJQQ-2029 20191210 end
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        //add by wuzhi.peng for liuhai switch
        if(onInterceptTouchEventForSwitchLiuhai(event)){
            return true;
        }
        //add by wuzhi.peng for liuhai switch
    ...
    private boolean handleQsTouch(MotionEvent event) {
        ...
        //modify by wangjian for EJQQ-2029 20191210 start
        //if (!mQsExpandImmediate && mQsTracking) {
        if (!mExtremeMode && !mQsExpandImmediate && mQsTracking) {
        //modify by wangjian for EJQQ-2029 20191210 end
            onQsTouch(event);
    ...
    private boolean shouldQuickSettingsIntercept(float x, float y, float yDiff) {
        ...
        View header = ((mKeyguardShowing || mQs == null) && !mExtremeMode) ? mKeyguardStatusBar : mQs.getHeader();
        //modify by wangjian for EJQQ-2029 20191210 end
        ...
        if (start) {
            ...
            if (mFalsingManager.shouldEnforceBouncer()) {
                mStatusBar.executeRunnableDismissingKeyguard(new Runnable() {
                    ...
                    public void run() {
                        //FACE_UNLOCK_SUPPORT start
                        if (FaceUnlockUtil.isFaceUnlockSupport() && FaceUnlockUtil.getInstance().launchCamera(mLastCameraLaunchSource)) {
    ...
    protected boolean onMiddleClicked() {
        ...
        if(FaceUnlockUtil.isFaceUnlockSupport() && FaceUnlockUtil.getInstance().startFaceUnlockByRetry(FaceUnlockUtil.FACE_UNLOCK_RETRY_IN_KEYGUARD)) {
            Log.d("NotificationpanelView", "onMiddleClicked startFaceUnlockByRetry");
            return true;
        }
        //FACE_UNLOCK_SUPPORT end
        switch (mBarState) {
    ...
    private boolean onInterceptTouchEventForSwitchLiuhai(MotionEvent event){

         if(event.getAction()==MotionEvent.ACTION_DOWN){
                float touchY = event.getRawY();
                float touchX = event.getRawX();
                mIntercept = mShowInterceptTouchEvent && (touchX<=getWidth()/4) && (touchY<=mStatusBarMinHeight) ;
        }
            if ( mIntercept){
                return true;
            } else {
       ...
       private final class SettingObserver extends ContentObserver {
        private final Uri LIUAHI_STATUS =
                Settings.System.getUriFor(Settings.System.SWIPE_LIUHAI_STATUS);
        private final Uri SWIPE_HIDDEN_LIUHAI =
                Settings.System.getUriFor(Settings.System.SWIPE_TOPLEFT_HIDDEN_LIUHAI);
        ...
        public void init() {
            mContext.getContentResolver().registerContentObserver(LIUAHI_STATUS, false, this);
            mContext.getContentResolver().registerContentObserver(SWIPE_HIDDEN_LIUHAI, false, this);
        }
    ...
    private boolean getLiuhaiInterceptSettingsValue(){

            return Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.SWIPE_LIUHAI_STATUS, 0)==0
                &&Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.SWIPE_TOPLEFT_HIDDEN_LIUHAI, 0)==1;
    }
    //add by wuzhi.peng for liuhai switch end
    //add by wangjian for EJQQ-2029 20191210 start

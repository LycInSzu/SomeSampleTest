...
public class StatusBarKeyguardViewManager implements RemoteInputController.Callback,
    ...
    private void updateLockIcon() {
        ...
        if (mLastLockVisible != lockVisible) {
            ...
            if (lockVisible) {
         // FACE_UNLOCK_SUPPORT start
        if (FaceUnlockUtil.getInstance().isAllowSetLockIconBg()) {
            FaceUnlockUtil.getInstance().setLockIconBg();
        }
        //FACE_UNLOCK_SUPPORT end
    ...
    public boolean onBackPressed(boolean hideImmediately) {
        if (mBouncer.isShowing()) {
            //add for EJSLYQ-66 by liyuchong 20200102 begin
            mStatusBar.userActivity();
            //add for EJSLYQ-66 by liyuchong 20200102 end
            mStatusBar.endAffordanceLaunch();
            // The second condition is for SIM card locked bouncer
            if (mBouncer.isScrimmed() && !mBouncer.needsFullscreenBouncer()) {
//                modify for EJQQ-1440 by liyuchong 20191108 begin
                /*hideBouncer(false);
                updateStates();*/
                mNotificationPanelView.resetViews(false /* animate */);
                reset(hideImmediately);
//                modify for EJQQ-1440 by liyuchong 20191108 end
            } else {
                reset(hideImmediately);
            }
    ...
    //add for EJQQ-1860 by liyuchong 20191206 begin
    private Runnable mMakeNavigationBarInVisibleRunnable = new Runnable() {
        @Override
        public void run() {
            if (mStatusBar.getNavigationBarView() != null) {
                mStatusBar.getNavigationBarView().getRootView().setVisibility(View.GONE);
            }else {
                mContainer.removeCallbacks(mMakeNavigationBarInVisibleRunnable);
                mContainer.postOnAnimationDelayed(mMakeNavigationBarInVisibleRunnable, 350);
            }
        }
    };
    //add for EJQQ-1860 by liyuchong 20191206 end
    ...
    protected void updateNavigationBarVisibility(boolean navBarVisible) {
        //modify for EJQQ-1860 by liyuchong 20191206 begin
/*        if (mStatusBar.getNavigationBarView() != null) {
            if (navBarVisible) {
        ...
        //modify for EJQQ-1860 by liyuchong 20191206 end
    }

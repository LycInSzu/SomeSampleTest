 ...
 */
public class SecretNoteSet extends NoteSet {
    ...
    public SecretNoteSet(Path path, NoteAppImpl application) {
        ...
        mItemPath = SecretNoteItem.SECRET_ITEM_PATH;
    ...
    private static NoteItem loadOrUpdateItem(Path path, Cursor cursor, DataManager dataManager,
        ...
        synchronized (DataManager.LOCK) {
            SecretNoteItem item = (SecretNoteItem) dataManager.peekNoteObject(path);
            if (item == null) {
                item = new SecretNoteItem(path, app, cursor);
            } else {
    ...
    public ArrayList<NoteItem> getNoteItem(int start, int count) {
        ...
        try {
            while (cursor.moveToNext()) {
                int id = cursor.getInt(SecretNoteItem.INDEX_ID);

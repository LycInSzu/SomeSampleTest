...
public class AutofitTextView extends TextView {
    ...
    private void refitText() {
        ...
        if (targetWidth > 0) {
            ...
            if ((mMaxLines == 1 && mPaint.measureText(text) > targetWidth) || getLineCount(text, mPaint, size, targetWidth, displayMetrics) > mMaxLines) {
    ...
    private static float getTextSize(String text, TextPaint paint, float targetWidth, int maxLines,float low, float high, float precision,DisplayMetrics displayMetrics) {
        ...
        if (SPEW) Log.e(TAG, "low=" + low + " high=" + high + " mid=" + mid + " target=" + targetWidth + " maxLines=" + maxLines + " lineCount=" + lineCount);

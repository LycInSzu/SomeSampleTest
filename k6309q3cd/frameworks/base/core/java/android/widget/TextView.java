...
//prize add by liyuchong, in Arabic Google message show phone number flipped ,20190228-begin
import java.util.Locale;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
//prize add by liyuchong, in Arabic Google message show phone number flipped ,20190228-end
/**
...
//prize add by liyuchong, in Arabic Google message show phone number flipped ,20190228-begin
	private boolean needLtrDirection(CharSequence text) {
	...
	private boolean moreContacts(CharSequence text) {
        String pattern="(^\\+|^)[0-9\\(\\) -]{3,}\\+[0-9]( رقم أخر)\\)";
        Pattern r = Pattern.compile(pattern);
        Matcher m = r.matcher(text);
        if (m.find()) {
            return true;
...
//prize add by liyuchong, in Arabic Google message show phone number flipped ,20190228-end
    private void setText(CharSequence text, BufferType type,
        ...
        if ("ar".equals(lan)) {
            if (needLtrDirection(text)) {
                this.setTextDirection(View.TEXT_DIRECTION_LTR);
            } else if ("com.google.android.apps.messaging".equals(getContext().getApplicationInfo().packageName)) {
				String ss = text.toString();
                if (specialBegin(text)) {
                    String[] s1 = ss.split("-");
                    StringBuilder temp = new StringBuilder();
                    temp.append(s1[0]).append("-").append("\u202D");
                    for (int i = 1; i < s1.length; i++) {
                        temp.append(s1[i]);
                    }
                    temp.append("\u202C");
                    text = temp.toString();
                } else if (moreContacts(text)) {
                    String[] s2 = ss.split("\\(");
                    ...
                    temp2.append(s2[s2.length-2]).append("\u202C").append("(").append(s2[s2.length - 1]);
					text = temp2.toString();
                }
            }
        }

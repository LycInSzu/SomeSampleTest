...
// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-begin
        int mPollingIntervalMs = 15;
        HashMap<Integer, ThreadControlInfo> mThreadControlInfo = new HashMap<>();

        class ThreadControlInfo {
            public int mModifiedCnt;
            public int mLastModifiedCnt;
            public boolean mShouldAbort;
        }

        class MonitorRunnable implements Runnable {
            int id;
            ThreadControlInfo info;

            public MonitorRunnable(int objId) {
                this.id = objId;
                this.info = mThreadControlInfo.get(id);
            }
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.w(TAG, "sleep fail\n");
                }

                while (true) {
                    if (info.mShouldAbort) {
                        Log.i(TAG, "thread out 1");
                        break;
                    }

                    synchronized (MtpStorageManager.this) {
                        Log.i(TAG, "in thread, info.mLastModifiedCnt " + info.mLastModifiedCnt + " info.mModifiedCnt" + info.mModifiedCnt);

                        if (info.mLastModifiedCnt != info.mModifiedCnt) {
                            info.mLastModifiedCnt = info.mModifiedCnt;
                            need_sleep = true;
                        } else {
                            need_sleep = false;
                            info.mLastModifiedCnt = info.mModifiedCnt = 0;
                            Log.w(TAG, "do sendObjecInfoChanged for " + id);
                            handleInfoChangedObject(id);
                        }
                    }
                    ...
                    if (need_sleep) {
                        ...
                        Log.i(TAG, "thread out 2");
                        break;
                    }
                }
            }
        }
// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-end

        MtpObjectObserver(MtpObject object) {
		// prize modified by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-begin
            super(object.getPath().toString(),
                    MOVED_FROM | MOVED_TO | DELETE | CREATE | IN_ONLYDIR | MODIFY);
		// prize modified by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-end
            mObject = object;
        }
        ...
        public void onEvent(int event, String path) {
            synchronized (MtpStorageManager.this) {
                ...
                if ((event & MOVED_TO) != 0 || (event & CREATE) != 0) {
				    ...
				    // prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-begin
                    if (mThreadControlInfo.containsKey(obj.getId())) {
                        mThreadControlInfo.get(obj.getId()).mShouldAbort = true;
                    }
					// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-end
				...
				// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-begin
                } else if ((event & MODIFY) != 0) {
                    if (sDebug)
                        Log.i(TAG, "Got inotify modified event for " + path + " " + event);

                    if (!mThreadControlInfo.containsKey(obj.getId())) {
                        mThreadControlInfo.put(obj.getId(), new ThreadControlInfo());
                    }

                    //avoid big file to cross the border
                    if (mThreadControlInfo.get(obj.getId()).mModifiedCnt >= 65535)
                        mThreadControlInfo.get(obj.getId()).mModifiedCnt = 1;
                    else
                        mThreadControlInfo.get(obj.getId()).mModifiedCnt++;


                    int lastModifiedCnt = mThreadControlInfo.get(obj.getId()).mLastModifiedCnt;
                    int modifiedCnt = mThreadControlInfo.get(obj.getId()).mModifiedCnt;

                    Log.i(TAG, "lastModifiedCnt " + lastModifiedCnt + " modifiedCnt" + modifiedCnt);

                    /* 1st time */
                    if (lastModifiedCnt == 0 && modifiedCnt == 1) {
                        new Thread(new MonitorRunnable(obj.getId())) {
                        }.start();
                    }
					// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-end
                } else {
	...
	// prize added by liyuchong, client bug 25990, MTK patch for mtp screenshot picture 0kb , 20190119-begin
    private synchronized void handleInfoChangedObject(int id) {
        MtpObject obj = getObject(id);
        MtpObjectState state = obj.getState();
        MtpOperation op = obj.getOperation();
        //switch (state) {
        //    case NORMAL:
        ...
        //    default:
        //        // This shouldn't happen; states correspond to objects that don't exist
        //        Log.e(TAG, "Got unexpected object info changed for " + obj.getName());
        //}
        mMtpNotifier.sendObjectRemoved(obj.getId());
        mMtpNotifier.sendObjectAdded(obj.getId());

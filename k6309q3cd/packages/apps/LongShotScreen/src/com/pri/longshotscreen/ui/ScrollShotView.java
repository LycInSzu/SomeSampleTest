package com.pri.longshotscreen.ui;//prize-modify the package name by xiekui-20181220-line
...
import com.pri.longshotscreen.utils.Log;
...
//prize-modify the package name by xiekui-20181220-start
import com.pri.longshotscreen.R;
import com.pri.longshotscreen.manager.NoticeParam;
import com.pri.longshotscreen.manager.SuperShotFloatViewManager;
import com.pri.longshotscreen.manager.SuperShotNotificationManager;
import com.pri.longshotscreen.utils.SuperShotUtils;
//prize-modify the package name by xiekui-20181220-end
...
public class ScrollShotView {
    ...
    public void setScreenSize(int width, int height) {
        ...
        //prize add by liuwei, bug id:68215,2018-11-27 begin
        if(mScreenWidth > mScreenHeight){
        // prize modified by liyuchong,  bug id 71492, change parameter to solve long screen shot problem of navigation bar in horizontal screen direction  , 20190301-begin
            if (Settings.Secure.getInt(mContext.getContentResolver(), "swipe_up_to_switch_apps_enabled",1)==0){
                mInjectEventStartPoint -= 5;
                mInjectEventEndPoint -= 5;
            }else {
                mInjectEventStartPoint += 30;
                mInjectEventEndPoint += 30;
            }
        // prize modified by liyuchong,  bug id 71492, change parameter to solve long screen shot problem of navigation bar in horizontal screen direction  , 20190301-end
        }
        //prize add by liuwei, bug id:68215,2018-11-27 end
    ...
    public void showScrollShotView() {
        ...
        mTmpBitmap = mTmpBitmap.copy(Bitmap.Config.ARGB_8888, true);
    ...
    public void stopScrollShot() {
        ...
        Intent i = new Intent("com.freeme.supershot.ScrollShot");
        i.setPackage("com.pri.longshotscreen");
    ...
    private void makeTopSurfaceViewToShotLongScreen() {
        ...
        mLongShotView = new LongShotView(mContext, mHandler, this,
                ...
                - mStatusBarHeight);
        //prize-add for fixbug:70612 by xiekui-20190115-start
        if (mContext.getResources().getBoolean(com.android.internal.R.bool.config_fillMainBuiltInDisplayCutout)) {
            mLayoutParams.layoutInDisplayCutoutMode = WindowManager.LayoutParams.LAYOUT_IN_DISPLAY_CUTOUT_MODE_NEVER;
            mRect = new Rect(0, 0, mScreenWidth, mScreenHeight);
            mLongShotView = new LongShotView(mContext, mHandler, this,
                0, mScreenWidth, mScreenHeight
                - mStatusBarHeight);
        }
        //prize-add for fixbug:70612 by xiekui-20190115-end
        ...
        NoticeParam localNoticeParam = new NoticeParam(10005,
                SuperShotUtils.getResourceString(mContext,
                        ...
                        SuperShotUtils.getResourceString(this.mContext,
                                ...
                                R.drawable.screenshot, SuperShotUtils.getResourceString(
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (mCounterLongShot == 1) {
                    ...
                    mTopAreaBitmap = mTopAreaBitmap.copy(Bitmap.Config.ARGB_8888, true);
                ...
                firstBitmap = firstBitmap.copy(Bitmap.Config.ARGB_8888, true);
                ...
                //prize add by liuwei, bug id:68067,2018-11-27 begin
                if(mCounterLongShot == 1 && mScreenHeight > mScreenWidth){
                    Bitmap checkMoveBitmap = Bitmap.createBitmap(mScreenBitmap, 0,
                        mInjectEventEndPoint, mScreenWidth,
                        mInjectEventStartPoint - mInjectEventEndPoint);
                    checkMoveBitmap = checkMoveBitmap.copy(Bitmap.Config.ARGB_8888, true);
                    if(checkMoveBitmap != null && checkMoveBitmap.sameAs(firstBitmap)){
                        mInjectEventStartPoint-=150;
                        mInjectEventEndPoint-=150;
                    }
                }
                //prize add by liuwei, bug id:68067,2018-11-27 end
            ...
            if (lineList.size() != 0) {
                if (true/*lineList.size() == 1*/) {
                    ...
                    if ((line_index <= (last1.getHeight() - 1))) {
                        ...
                        last = last.copy(Bitmap.Config.ARGB_8888, true);
        ...
        this.mHandler.postDelayed(new Runnable() {
            public void run() {
                ...
                Bitmap aBitmap = Bitmap.createBitmap(mScreenBitmap, 0,
                        mInjectEventEndPoint, mScreenWidth,
                        mInjectEventStartPoint - mInjectEventEndPoint);
                aBitmap = aBitmap.copy(Bitmap.Config.ARGB_8888, true);
                ...
                mBottomAreaBitmap = mBottomAreaBitmap.copy(Bitmap.Config.ARGB_8888, true);      
    ...
    private void injectPointerEvent(float fromX, float toX, float fromY, float toY) {
        Log.d(TAG, "[injectPointerEvent]    mIsReachBottom = "+mIsReachBottom+
                ...
                "   toX = "+toX+
                "   fromY = "+fromY+
                "   toY = "+toY+" ScreenHeight:"+mScreenHeight+" ScreenWidth:"+mScreenWidth);//mScreenHeight < mScreenWidth
        try {
...

            {
                ...
                }
                {
                    ...
                    //prize add by liuwei, bug id:68090,2018-11-27 begin
                    //y -=scorllShotViewRemainheight; 

                    // prize modified by liyuchong,  bug id 71492, change parameter to solve long screen shot break problem   , 20190301-begin
                    if(mScreenHeight < mScreenWidth){
                      y -= 2;
                    }else{
                      y -=22;
                    }
                    // prize modified by liyuchong,  bug id 71492, change parameter to solve long screen shot break problem   , 20190301-end
                    //prize add by liuwei, bug id:68090,2018-11-27 end

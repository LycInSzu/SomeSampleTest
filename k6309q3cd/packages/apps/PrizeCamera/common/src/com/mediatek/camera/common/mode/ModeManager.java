    ...
    private class FeatureLoadListener implements FeatureLoadDoneListener {
        ...
        public void onBuildInLoadDone(String cameraId, CameraApi cameraApi) {
            ...
            if (cameraApi.equals(mCurrentCameraApi)) {
                ...
                /*prize-modify-switch mode after switch camera-xiaoping-20180913-start*/
                if (String.valueOf(FRONT_CAMERAID).equals(cameraId)) {
                    for (int i = 0; i < modeItems.size(); i++) {
                        if (modeItems.get(i).mClassName.equals(PictureZoomModeEntry.class.getName()) || modeItems.get(i).mClassName.equals(LowLightModeEntry.class.getName())) {
                            modeItems.remove(modeItems.get(i));
                        }
                    }
                }

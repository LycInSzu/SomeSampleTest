...
/*prize-add-huangpengfei-2018-10-31-start*/
import com.mediatek.camera.common.debug.LogHelper;
import com.mediatek.camera.common.debug.LogUtil;
/*prize-add-huangpengfei-2018-10-31-end*/
...
public class DataStore {
	//prize-add-huangpengfei-2018-10-31
    private static final LogUtil.Tag TAG = new LogUtil.Tag(DataStore.class.getSimpleName());
    ...
    public void resetSettingsData(){
        LogHelper.i(TAG, "[resetSettingsData] ");
        SharedPreferencesWrapper prefWrapperForGlobalSavingTimestamp
                = getSharedPreferencesWrapperSync(GLOBAL_SCOPE + PREFIX_SAVING_TIMESTAMP);
        SharedPreferences prefSavingTimestamp = prefWrapperForGlobalSavingTimestamp.getPreferences();
        SharedPreferences.Editor prefSavingTimestampEdit = prefSavingTimestamp.edit();
        prefSavingTimestampEdit.clear();
        boolean commit = prefSavingTimestampEdit.commit();
        LogHelper.i(TAG, "[resetSettingsData] prefSavingTimestampEdit " + commit);

        SharedPreferencesWrapper prefWrapperForGlobal
                = getSharedPreferencesWrapperSync(GLOBAL_SCOPE);
        SharedPreferences prefForGlobal = prefWrapperForGlobal.getPreferences();
        SharedPreferences.Editor prefForGlobalEdit = prefForGlobal.edit();
        prefForGlobalEdit.clear();
        commit = prefForGlobalEdit.commit();
        LogHelper.i(TAG, "[resetSettingsData] prefForGlobalEdit " + commit);

        for (int i = 0;i < 2;i++){
            SharedPreferencesWrapper sharedPreferencesWrapperSync = getSharedPreferencesWrapperSync(getCameraScope(i));
            SharedPreferences preferences = sharedPreferencesWrapperSync.getPreferences();
            SharedPreferences.Editor edit = preferences.edit();
            edit.clear();
            commit = edit.commit();
            LogHelper.i(TAG, "[resetSettingsData] i = " + i + " commit = " + commit);
            if (i == 0){
                SharedPreferencesWrapper prefWrapperForSavingTimestamp
                        = getSharedPreferencesWrapperSync(getCameraScope(i) + PREFIX_SAVING_TIMESTAMP);
                SharedPreferences preferencesTimestamp = prefWrapperForSavingTimestamp.getPreferences();
                SharedPreferences.Editor editTimestamp = preferencesTimestamp.edit();
                editTimestamp.clear();
                boolean commitTimestamp = editTimestamp.commit();
                LogHelper.i(TAG, "[resetSettingsData]" + " commitTimestamp = " + commitTimestamp);
            }
        }
    }
    public void clearCache(){
        ...
        for (int i = 0;i < 2;i++){
            SharedPreferencesWrapper sharedPreferencesWrapperSync = getSharedPreferencesWrapperSync(getCameraScope(i));
            sharedPreferencesWrapperSync.clearCache();
        }
    }
	/*prize-add-huangpengfei-2018-10-31-end*/

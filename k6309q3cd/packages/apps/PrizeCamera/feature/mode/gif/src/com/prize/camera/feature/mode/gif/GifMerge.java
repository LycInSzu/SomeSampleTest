...
import com.lava.camera.feature.mode.gif.encode.AnimatedGifEncoder;
...
//  private byte[][] mImages = null;
    ...
    public GifMerge(IApp app, String path, String fileName, int delay, int maxSize, int w, int h, GifCallback callback, Location location){
        mApp = app;
        mFilePath = path;
        mDelay = delay;
        mCallback = callback;
    ...
    public void addBitmap(Bitmap bitmap){
        if(null != bitmap && mProcessedTime < MAX_DUAL_TIME && mCurrentBitmap == null){
            ...
            //mSingle[mAddCursor] = true;

            if(mProcessedTime == 0){
                mFirstBitmap = mCurrentBitmap;
            }

            //mAddCursor++;

            LogHelper.d(TAG, "lava gif shot addBitmap mAddCursor="+ mProcessedTime/1000);
        }else{
            LogHelper.d(TAG, "lava gif shot addBitmap error");
        }
    }
        ...
        while (mCanRun) {
            ...
            if(mProcessedTime< MAX_DUAL_TIME && !isCurrentMerged  && mCurrentBitmap != null){
                if(!mInited){
                    ...
                    try {
                        //result = mGifEncoder.init(mFilePath + TEMP_FILE_END, mWidth, mHeight, 0x100, 0x3c, mDelay);
                   ...
                   Bitmap bitmap = mCurrentBitmap;
                    ...
                    try {
                        ...
                        // TODO: handle exception
                        LogHelper.e(TAG, "gif addframe error:"+e);
                        onGifEncodeError(false);
                        return;
                    }
    ...
    private void saveImageToDatabase() {
        ...
        try {
            ...
            if (mUri != null) {
                ...
                Bitmap thumbmailBitmap =  Bitmap.createScaledBitmap(mFirstBitmap, ThumbnailHelper.getThumbnailHeight(), ThumbnailHelper.getThumbnailWidth(), true);

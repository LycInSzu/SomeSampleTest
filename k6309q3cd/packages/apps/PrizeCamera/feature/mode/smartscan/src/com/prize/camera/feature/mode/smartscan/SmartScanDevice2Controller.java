...
public class SmartScanDevice2Controller extends Device2Controller implements ISmartScanDeviceController ,
        CaptureSurface.ImageCallback,
        ISettingManager.SettingDevice2Requester {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(SmartScanDevice2Controller.class.getSimpleName());
    private static final int CAPTURE_FORMAT = ImageFormat.JPEG;
    private static final int CAPTURE_MAX_NUMBER = 2;
    private static final int WAIT_TIME = 5;

    private static final String LAVA_PORTRAIT_KEY = "com.mediatek.arcsoftrelighting.switch";
    private static final String LAVA_PORTRAIT_VALUE_KEY = "com.mediatek.arcsoftrelighting.lightmode";
    private CaptureRequest.Key<int[]> mLavaPortraitKey = null;
    private CaptureRequest.Key<int[]> mLavaPortraitValueKey = null;
    private static final int[] LAVA_PORTRAIT_KEY_VALUE = new int[]{1};
    private static int[] CURRENT_PORTRAIT_VALUE = new int[]{0};
    ...
    public SmartScanDevice2Controller(@Nonnull Activity activity, @Nonnull ICameraContext context){
    ...
    public void openCamera(DeviceInfo info) {
        ...
        if (canOpenCamera(cameraId)){
            try {
                ...
                mLavaPortraitKey = CameraUtil.getRequestKey(
                        mCameraCharacteristics, LAVA_PORTRAIT_KEY);
                mLavaPortraitValueKey = CameraUtil.getRequestKey(
                        mCameraCharacteristics, LAVA_PORTRAIT_VALUE_KEY);
    ...
    @Override
    public void setPortraitValueChanged(int value, boolean isNeedRepeatingRequest) {
        if (mCurrentValue != value){
            mCurrentValue = value;
            if (isNeedRepeatingRequest) {
    ...
    private void initSettings() throws CameraAccessException {
        ...
        Relation relation = SmartScanRestriction.getRestriction().getRelation("on",
                false);
    ...
    private void setSpecialVendorTag(CaptureRequest.Builder builder) {
        if (mLavaPortraitKey != null) {
            builder.set(mLavaPortraitKey, LAVA_PORTRAIT_KEY_VALUE);
            LogHelper.d(TAG, "[setSpecialVendorTag] set portrait key.");
        }
        if (mLavaPortraitValueKey != null) {
            CURRENT_PORTRAIT_VALUE[0] = mCurrentValue;
            builder.set(mLavaPortraitValueKey, CURRENT_PORTRAIT_VALUE);
            LogHelper.d(TAG, "[setSpecialVendorTag] portrait value " + mCurrentValue);
        }
    }

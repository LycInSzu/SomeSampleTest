...
package com.prize.camera.feature.mode.smartscan.view;
...
public final class ViewfinderView extends View {
  ...
  public void onDraw(Canvas canvas) {
    ...
    if(scannerStart == 0 || scannerEnd == 0) {
      scannerStart = mPreviewFrame.top;
      scannerEnd = mPreviewFrame.bottom;
    }

    int width = canvas.getWidth();
    int height = canvas.getHeight();
    // Draw the exterior (i.e. outside the framing rect) darkened
    drawExterior(canvas, mPreviewFrame, width, height);


    if (resultBitmap != null) {
      // Draw the opaque result bitmap over the scanning rectangle
      paint.setAlpha(OPAQUE);
      canvas.drawBitmap(resultBitmap, mPreviewFrame.left, mPreviewFrame.top, paint);
    } else {
      // Draw a two pixel solid black border inside the framing rect
      drawFrame(canvas, mPreviewFrame);
      // 绘制边角
      drawCorner(canvas, mPreviewFrame);
      //绘制提示信息
      drawTextInfo(canvas, mPreviewFrame);
      // Draw a red "laser scanner" line through the middle to show decoding is active
      drawLaserScanner(canvas, mPreviewFrame);
      ...
      if (currentPossible.isEmpty()) {
        ...
        for (ResultPoint point : currentPossible) {
          canvas.drawCircle(mPreviewFrame.left + point.getX(), mPreviewFrame.top + point.getY(), 6.0f, paint);
        }
      }
      if (currentLast != null) {
        ...
        for (ResultPoint point : currentLast) {
          canvas.drawCircle(mPreviewFrame.left + point.getX(), mPreviewFrame.top + point.getY(), 3.0f, paint);
        }
      }
      ...
      //指定重绘区域，该方法会在子线程中执行
      postInvalidateDelayed(ANIMATION_DELAY, mPreviewFrame.left, mPreviewFrame.top, mPreviewFrame.right, mPreviewFrame.bottom);
    }
  }
...
//      canvas.drawRect(frame.left, scannerStart, frame.right, scannerStart + SCANNER_LINE_HEIGHT, paint);
  ...
  public void setmPreviewFrame(Rect frame){
    mPreviewFrame = frame;
  }

...
public class HdrViewController {
    ...
    /*prize-modify-add hdr to turn off the tips-xiaoping-20181029-start*/
    private IAppUi.HintInfo mHdrHintInfo;
    /*prize-modify-add hdr to turn off the tips-xiaoping-20181029-end*/
    /**
     * The constructor.
     * @param app IApp.
     * @param hdr Hdr.
    ...
    public HdrViewController(IApp app, Hdr hdr) {
        ...
        /*prize-modify-add hdr to turn off the tips-xiaoping-20181029-start*/
        mHdrHintInfo = new IAppUi.HintInfo();
        mHdrHintInfo.mDelayTime = 2000;
        int id = mApp.getActivity().getResources().getIdentifier("hint_text_background",
                "drawable", mApp.getActivity().getPackageName());
        mHdrHintInfo.mBackground = mApp.getActivity().getDrawable(id);
        mHdrHintInfo.mType = IAppUi.HintType.TYPE_AUTO_HIDE;
        /*prize-modify-add hdr to turn off the tips-xiaoping-20181029-end*/
    }
        ...
        public void onClick(View view) {
            ...
            if (mHdr.getEntryValues().size() > HDR_ENTRY_LIST_SWITCH_SIZE) {
                ...
                if (HDR_ON_VALUE.equals(value)) {
                    mHdrHintInfo.mHintText = mApp.getActivity().getString(R.string.hdr_open_tips_on);
                    if (FeatureSwitcher.getCurrentProjectValue() == 2) {
                        mHdrHintInfo.mHintText = mApp.getActivity().getString(R.string.hdr_guide_capture); 
                    }
                } else if (HDR_OFF_VALUE.equals(value)) {
                    mHdrHintInfo.mHintText = mApp.getActivity().getString(R.string.hdr_open_tips_off);
                }
                if (!(FeatureSwitcher.getCurrentProjectValue() == 2 && HDR_OFF_VALUE.equals(value))) {
                    mApp.getAppUi().showScreenHint(mHdrHintInfo,(int) mApp.getActivity().getResources().getDimension(R.dimen.hdr_tips_margin_top));
                }
                /*prize-modify-add hdr to turn off the tips-xiaoping-20181029-end*/
    ...
    private void updateHdrEntryView(String value) {
        ...
        if (HDR_ON_VALUE.equals(value)) {
            mHdrEntryView.setImageResource(R.drawable.prize_ic_hdr_on);//prize-change icon name-huangpengfei-2018-9-27
            ...
            mHdrEntryView.setImageResource(R.drawable.prize_ic_hdr_off);//prize-change icon name-huangpengfei-2018-9-27
            mHdrEntryView.setContentDescription(mApp.getActivity().getResources().getString(
                    R.string.accessibility_hdr_off));
        }
        /*prize-modify-bugid:68729 Camera restore default settings off hdr-xiaoping-20181128-start*/
        mApp.getAppUi().setHdrValue(value);

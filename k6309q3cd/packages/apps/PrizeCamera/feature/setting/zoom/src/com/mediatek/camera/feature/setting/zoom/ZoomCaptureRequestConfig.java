...
public class ZoomCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure,
    ...
    private Rect cropRegionForZoom(float ratio) {
        //prize-modify-bug:68967 add Increase judgment of NullPointerException
        if (mSensorRect != null) {
            ...
            LogHelper.i(TAG,"ratio: "+ratio+",left: "+(xCenter - xDelta)+",top: "+(yCenter - yDelta)+",right: "+(xCenter + xDelta)+",bottom: "+(yCenter + yDelta));
            return new Rect(xCenter - xDelta, yCenter - yDelta, xCenter + xDelta, yCenter + yDelta);
        } else {
            LogHelper.w(TAG,"mSensorRect is null ,set the default Rect");
    ...
    private float calculateZoomRatio(double distanceRatio) {
		...
		//prize-change-haungpengfei-2018-10-24
        float curRatio = (float) (mBasicZoomRatio + /*(maxRatio - minRatio) * distanceRatio*/distanceRatio);

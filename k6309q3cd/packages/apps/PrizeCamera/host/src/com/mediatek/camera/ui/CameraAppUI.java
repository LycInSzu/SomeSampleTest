...
public class CameraAppUI implements IAppUi,IArcProgressBarUI,PrizeSeekBar.OnProgressChangeListener {
    ...
    public void onCreate() {
        ...
        PrizePluginModeManager pmm = new PrizePluginModeManager(mApp,mScreenPixWidth);
...
//        mShutterManager.clearShutterButtons();
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if(item instanceof IAppUi.PluginModeItem && !item.mClassName.equals(pluginName)){
                continue;
            }
            ...
            if (item.mType.equals("Picture")) {
                ...
                LogHelper.i(TAG, "zhangguo videomode");
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
            ...
            //prize-tangan-20180921-add prizecamera-end
                ...
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_smart_scan), item.mType, PLUGIN_MODE_PRIORITY, ShutterButtonManager.MODE_SCAN);
            }else if(item.mType.equals("HDR")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, FeatureSwitcher.isArcsoftNightShotSupported() ? 1: 0, ShutterButtonManager.MODE_HDR);
            }else if(item.mType.equals("UHD")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, FeatureSwitcher.getVideoModeIndex() + 2,ShutterButtonManager.MODE_PICTURE_ZOOM);
            }else if(item.mType.equals("LowLight")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, 0,ShutterButtonManager.MODE_LOWLIGHT_SHOT);
            }else if(item.mType.equals("Professional")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, PLUGIN_MODE_PRIORITY,ShutterButtonManager.MODE_PROFESSIONAL);
            }else if(item.mType.equals("Filter")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, PLUGIN_MODE_PRIORITY, ShutterButtonManager.MODE_FILTER);
            }else if(item.mType.equals("Gif")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_photo), item.mType, PLUGIN_MODE_PRIORITY, ShutterButtonManager.MODE_GIF);
            }else if(item.mType.equals("SlowMotion")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                        .getDrawable(
                                R.drawable.btn_video), item.mType, PLUGIN_MODE_PRIORITY, ShutterButtonManager.MODE_SLOWMOTION);
            }else if(item.mType.equals("FaceBeauty")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                                .getDrawable(R.drawable.btn_photo), item.mType,
                        FeatureSwitcher.isArcsoftSupperZoomSupported() ? FeatureSwitcher.getVideoModeIndex() + 3 : FeatureSwitcher.getVideoModeIndex() + 2,ShutterButtonManager.MODE_FACE_BEAUTY);
            }else if(item.mType.equals("PrizePano")){
                mShutterManager.registerShutterButton(mApp.getActivity().getResources()
                                .getDrawable(R.drawable.btn_photo), item.mType,
                        PLUGIN_MODE_PRIORITY, ShutterButtonManager.MODE_PRIZE_PANO);
            }
        }
    ...
    public void setPreviewSize(final int width, final int height,
        ...
        mGridManager.updatePreviewSize(width,height);//prize-add for GridLine -huangpengfei-2018-9-26
        ...
        mApp.getActivity().runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(null != mPrizeCircleView){
                    mPrizeCircleView.updatePosition(CameraAppUI.this.getSurfaceTextureView());
                }
            }
        });
    }
...
//prize-added by tangan-custom ui-begin
    ...
    public void addCameraSwitch(View view) {
        ...
        if(camera_switch.getChildCount() < 1){
            if(view != null){
                mApp.getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private boolean canShowCameraSwitcher(){
        if (getModeItem().mModeTitle == ModeTitle.LOWLIGHT || getModeItem().mModeTitle == ModeTitle.PICTUREZOOM ||
                getModeItem().mModeTitle == ModeTitle.PROFESSIONAL || getModeItem().mModeTitle == ModeTitle.APERTURE){
            return false;
        }else{
            return true;
        }
...
//            mModeChangeListener.onModeSelected(item.mClassName);
        ...
        if (mTimerTask == null) {
            mTimerTask = new PrizeTimeTask() {
                ...
                public void run() {
                    ...
                    if ( mLiftCameraState == LiftCamera.LiftCameraState.START_UP && mTimeInterval *index >3000 &&   Integer.valueOf(mPrizeReadNodeValue.getUpValue()) < 200) {
                    ...
                    if (mTimeInterval *index  > 1500 && Math.abs(mCurrentUpValue - Integer.valueOf(mPrizeReadNodeValue.getUpValue())) <=2
                            && (Integer.valueOf(mPrizeReadNodeValue.getUpValue()) >300  || Integer.valueOf(mPrizeReadNodeValue.getUpValue()) < 70)) {
                        ...
                        if (mImplLifaCamera.getLiftCameraState() != LiftCamera.LiftCameraState.END
                                && ((mLiftCameraState == LiftCamera.LiftCameraState.START_UP && maxUpValue >300)) || (mLiftCameraState == LiftCamera.LiftCameraState.START_DOWN && maxUpValue <70)) {
                            mImplLifaCamera.updateLiftCameraState(LiftCamera.LiftCameraState.END);
                        }
                    }
                    mCurrentUpValue = Integer.valueOf(mPrizeReadNodeValue.getUpValue());
                    if (index == 0) {
                        mFirstUpValue = mCurrentUpValue;
                    }
                    if ( mLiftCameraState == LiftCamera.LiftCameraState.END && (maxUpValue - mCurrentUpValue) >150 && !isPreventCamera) {
                        LogHelper.i(TAG,"maxUpValue: "+maxUpValue+",mCurrentUpValue: "+mCurrentUpValue);
    ...
    class ImplLifaCamera implements LiftCamera {
        ...
        public void updateLiftCameraState(LiftCameraState liftCameraState) {
            ...
            if (liftCameraState == LiftCameraState.END) {
                ...
                mFirstUpValue = 0;
                if (mCameraId == 0) {
                    mConfigUIHandler.sendEmptyMessageDelayed(STOP_READ_NODEVALUE,50);
                } else {
    ...
    public void selectPluginMode(String mode, boolean scroll){

...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
	...
	/* PRIZE-PRIZE-E9 bug 472-wanzhijuan-2015-5-8-start */
	/** Listening to the headset button **/
	private MediaReceiver mMediaReceiver;
	/** MediaButtonReceiver Receives the headset button, pass to the action of ACTION_MEDIA_HARDKEY radio processing **/
	public static final String ACTION_MEDIA_HARDKEY = "com.prize.MEDIA_HARDKEY";

	/* PRIZE-PRIZE-E9 bug 472-wanzhijuan-2015-5-8-end */
	//FmSuspend feature
    private AlarmManager mAlarmManager;
    static final String NOTIFICATION_CHANNEL_FMRADIO = "fm_notification_channel";
    private PendingIntent mSendIntent;
	private static String ALARM_KEYER_ACTION = "com.prize.fm.alarm_keyer_action";
    @Override
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ((SOUND_POWER_DOWN_MSG.equals(action) && CMDPAUSE.equals(command))) {
                ...
                if ((0 == mValueHeadSetPlug) && isActivityForeground() &&
                	...
                	if (!isAirplaneMode() && !isUsbPlugged()) {
			...
			/*PRIZE-bug:1471-wanzhijuan-2015-5-29-start*/
			// Listening to the flight mode switch
			else if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)) {
				boolean isAirPlaneMode = intent.getBooleanExtra("state", false);
				if (isAirPlaneMode) {
					LogTools.d(TAG, ">>>ACTION_AIRPLANE_MODE_CHANGED");
					mFmServiceHandler.removeCallbacksAndMessages(null);
					exitFm();
					stopSelf();
				} 
				/*else {
					if (!mIsPowerUp) {
						tuneStation(getFrequency());
					}
				} */
			       
			} 
			/*PRIZE-bug:1471-wanzhijuan-2015-5-29-end*/
			else {
    ...
    private void usbChangeAsync() {
    	Log.i(TAG, "usbChangeAsync mPluggedIn=" + mPluggedIn);
        mFmServiceHandler.removeMessages(FmListener.MSGID_FM_USB_CHANGED);
        mFmServiceHandler.sendEmptyMessage(FmListener.MSGID_FM_USB_CHANGED);
    }
    
    private void usbChangeHandle() {
    	Bundle bundle = new Bundle();
		bundle.putInt(FmListener.CALLBACK_FLAG,
                FmListener.MSGID_FM_USB_CHANGED);
        bundle.putBoolean(FmListener.KEY_IS_SWITCH_USB, mPluggedIn);
        notifyActivityStateChanged(bundle);
		if (isPowerUp()) {
			if (sExitListener != null) {
				sExitListener.onExit();
	...
	private void registerUsbReceiver() {
		if (isUsb()) {
			...
			mPluggedIn = getUsbPlugged(batteryStatus);
		}
	}
	
	private boolean getUsbPlugged(Intent it) {
		int status = it.getIntExtra(BatteryManager.EXTRA_STATUS,
                BatteryManager.BATTERY_STATUS_UNKNOWN);
		Log.i(TAG, "getUsbPlugged status=" + status);
    ...
    private void setForceUse(boolean isSpeaker) {
		...
		if (callback != null) {
			Log.w(TAG, "setForceUse mIsSpeakerUsed = " + mIsSpeakerUsed);
			callback.updatespeakerRes(mIsSpeakerUsed);
		}
    }
    ...
    /**
	 * 
	 * To switch to the headphones sound when PowerDown
	 */
	private void setEarPhone() {
    ...
    private boolean powerDown() {
		...
		if (mIsSpeakerUsed) {
			setEarPhone();
			if (callback != null) {
				callback.updatespeakerRes(false);
			}
		}
        setMute(true);
        setRds(false);
	...
	public boolean isPowerUp() {
		boolean isPowerUp = mPowerStatus == POWER_UP;
		return isPowerUp;
	}
	...
	public boolean isPowerUping() {
		boolean isPowerUping = mPowerStatus == DURING_POWER_UP;
		return isPowerUping;
	}
	
	public boolean isPowerDown() {
		boolean isPowerDown = mPowerStatus == POWER_DOWN;
		return isPowerDown;
    }
    ...
    private boolean tuneStation(float frequency) {
        ...
        if (!isAntennaAvailable() || isAirplaneMode() || isUsbPlugged()) {
            return false;
        }
	...
	/** prize-add-Increase automatically shut off the radio-bu-zhongweilin-20150426-start **/
	/**
	 * Set automatically shut off the radio
	 */
	private BroadcastReceiver mFmSettingReceiver = new BroadcastReceiver() {
		@Override
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			if (action.equals("com.prize.fmsetting")) {
				int setting_time = intent.getIntExtra(
						FmSettings.FM_AUTO_OFF_SETTING_TIME, 0);
				Log.d(TAG, ">>> mFmSettingReceiver");
                timedShutdown(setting_time);
            } else if (action.equals(ALARM_KEYER_ACTION)) {
	...
	/**
	 * Set the timer and set the timer handler to monitor and send off the radio
	 */
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			Log.d(TAG, "<<< FmRadioService>>>>>>>>>>>>%%%%%%%%%%%%%%");
			timedShutdown(0);
			Intent mIntent = new Intent("com.prize.activity.finish");
			sendBroadcast(mIntent);
			resetAutoStop();
			exitFm();
			/*PRIZE-bug:2968-wanzhijuan-2015-7-28-start*/
			stopSelf();
			/*PRIZE-bug:2968-wanzhijuan-2015-7-28-end*/
	...
	private void resetAutoStop() {
		...
		editor.putInt(FmSettings.FM_AUTO_OFF_SETTING_TIME, FmSettings.AUTO_EXIT_STOP);
		editor.commit();
	}
	 ...
	 * Set the timer task
	 */
	private void setTimerTask(long delay) {
        if (mTimer != null) {
            try {
                mTimer.cancel();
            } catch (Exception e) {
	...
	/**
	 * Registry Settings automatically shut off the radio
	 */
	public void registerBoradcastReceiver() {
		IntentFilter myIntentFilter = new IntentFilter();
		myIntentFilter.addAction("com.prize.fmsetting");
        myIntentFilter.addAction(ALARM_KEYER_ACTION);
		registerReceiver(mFmSettingReceiver, myIntentFilter);
	}

	/**
	 * Cancel setup automatically shut off the radio
	 */
	...
	/** prize-add-Increase automatically shut off the radio-bu-zhongweilin-20150426-end **/
    @Override
    public void onCreate() {
		...
		mySharedPreferences = getSharedPreferences(
				FmSettings.FM_AUTO_OFF_SETTING, Activity.MODE_PRIVATE);
		int delValue = mySharedPreferences.getInt(
				FmSettings.FM_AUTO_OFF_SETTING_TIME, 0);
        ...
        if (FmUtils.isFmSuspendSupport()) {
            ...
            mSendIntent = PendingIntent.getBroadcast(this, 0, intent , PendingIntent.FLAG_UPDATE_CURRENT);
        }
		if (delValue != 0) {
            timedShutdown(delValue);
        }
        initAudioRecordSink();
        createRenderThread();
	...
	private void timedShutdown(int delValue) {
        Log.i(TAG, "timedShutdown delValue=" + delValue);
        if (FmUtils.isFmSuspendSupport()) {
            if (mAlarmManager != null) {
                mAlarmManager.cancel(mSendIntent);
            }
            if (delValue != 0) {
                mAlarmManager.setExact(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + delValue * 60 * 1000, mSendIntent);
            }
        } else {
            setTimerTask(delValue);
        }
    }

    /**Listening RingerMode change**/
    private RingerModeChangeReceiver mRingerModeChangeReceiver;
    /** Whether the vibration mode**/
    private boolean mIsRingerModeVibrate;
    private boolean mIsOffHook;
	private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
		...
		public void onCallStateChanged(int state, String incomingNumber) {
			...
			LogTools.i(TAG, "onCallStateChanged() state=" + state);
			switch (state) {
			case TelephonyManager.CALL_STATE_IDLE:
				if (mIsRingerModeVibrate && !mIsOffHook) {
					mAudioFocusChangeListener.onAudioFocusChange(AudioManager.AUDIOFOCUS_GAIN);
				}
				mIsOffHook = false;
				break;
			...
			case TelephonyManager.CALL_STATE_RINGING:
				if (mIsRingerModeVibrate) {
					mAudioFocusChangeListener.onAudioFocusChange(AudioManager.AUDIOFOCUS_LOSS_TRANSIENT);
				}
				break;
			}
		}
    ...
    private void addCallStateListener() {
    	...
    	//  Set the phone status listener
    	tm.listen(mPhoneStateListener, PhoneStateListener.LISTEN_CALL_STATE);
    }
    
    private void initAudioFocus() {
        mIsRingerModeVibrate = (mAudioManager.getRingerMode() == AudioManager.RINGER_MODE_VIBRATE);
        mRingerModeChangeReceiver = new RingerModeChangeReceiver();
        mRingerModeChangeReceiver.register();
    ...
    private class RingerModeChangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
        	if (intent != null) {
        		String action = intent.getAction();
        		if (!TextUtils.isEmpty(action) && action.equals(AudioManager.RINGER_MODE_CHANGED_ACTION)) {
        			mIsRingerModeVibrate = (intent.getIntExtra(AudioManager.EXTRA_RINGER_MODE, AudioManager.RINGER_MODE_NORMAL) == AudioManager.RINGER_MODE_VIBRATE);
        		}
        	}
        }
    }
	 ...
	 * 
	 **
	 * Receive MediaButtonReceiver sent the headset button information
	 * 
	 * @author wanzhijuan
	 * @version V1.0
	 */
	private class MediaReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			String action = intent.getAction();
			if (ACTION_MEDIA_HARDKEY.equals(action)) {
				// The KeyEvent object
				...
				if (keyAction == KeyEvent.ACTION_DOWN) {
					if ((KeyEvent.KEYCODE_HEADSETHOOK == keyCode || KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE == keyCode)) {
						LogTools.i(TAG, "MediaReceiver mIsInRecordingMode="
								+ mIsInRecordingMode);
						seekStationAsync(
									FmUtils
	...
	/**
	 * 
	 * Registered MediaReceiver broadcasting
	 */
	private void registerMediaReceiver() {
		IntentFilter filter = new IntentFilter(ACTION_MEDIA_HARDKEY);
	...
	/**
	 * 
	 * Cancel the registration MediaReceiver broadcast
	 */
    ...
    public void onDestroy() {
		...
		if (mRingerModeChangeReceiver != null) {
        	mRingerModeChangeReceiver.unregister();
        }
        abandonAudioFocus();
//		exitFm();
        if (null != mFmRecorder) {
    ...
    private void showNotification(String stationName, String radioText) {
    	RemoteViews views = new RemoteViews(getPackageName(), R.layout.statusbar);
            ...
            if (null == mNotificationBuilder) {
                mNotificationBuilder = new Notification.Builder(mContext,
                                                    NOTIFICATION_CHANNEL_FMRADIO);
            mNotificationBuilder.setSmallIcon(R.drawable.fm_title_icon);
        ...
        views.setOnClickPendingIntent(R.id.btn_stop, pIntent);
        ...
        views.setImageViewResource(R.id.right_icon, R.drawable.fm_title_icon);
            // Show FM Radio if empty
            if (TextUtils.isEmpty(stationName)) {
                stationName = getString(R.string.app_name);
            }
        views.setTextViewText(R.id.title, stationName);
    ...
    public boolean requestAudioFocus() {
		...
		//prize-speaker mode auto to earphone mode after notificaiton-tangan020180301-begin
		...
		mIsAudioFocusHeld = (AudioManager.AUDIOFOCUS_REQUEST_GRANTED == audioFocus);
		return mIsAudioFocusHeld;
		//prize-speaker mode auto to earphone mode after notificaiton-tangan020180301-end
    }
	...
	/**
	 * 
	 * To determine whether mobile flight mode
	 * @return boolean true Is the flight mode
	 */
    ...
    private void updateAudioFocus(int focusState) {
        ...
        switch (focusState) {
            ...
            case AudioManager.AUDIOFOCUS_GAIN:
    			...
    			if (callback != null) {
                    Log.w(TAG, "AUDIOFOCUS_GAIN mIsSpeakerUsed = " + mIsSpeakerUsed);
    				callback.updatespeakerRes(mIsSpeakerUsed);
    			}
                if (getForceUse() != AudioSystem.FORCE_SPEAKER && mIsSpeakerUsed) {
                     AudioSystem.setForceUse(FOR_PROPRIETARY, getForceUse());
                     mIsSpeakerUsed = false;
				} else if (getForceUse() == AudioSystem.FORCE_SPEAKER
						&& !mIsSpeakerUsed) {
                     AudioSystem.setForceUse(FOR_PROPRIETARY, getForceUse());
                     mIsSpeakerUsed = true;
				}
	...
	/**
	 * 
	 * Registered MediaButtonReceiver broadcasting
	 */
	private void registerMediaButton() {
		LogTools.i(TAG, "registerMediaButton() ");
		mCn = new ComponentName(getPackageName(),
				MediaButtonReceiver.class.getName());
		mAudioManager.registerMediaButtonEventReceiver(mCn);
	}

	/**
	 * 
	 * Cancel the registration MediaButtonReceiver broadcast
	 */
	private void unregisterMediaButton() {
		LogTools.i(TAG, "unregisterMediaButton() ");
		if (mCn != null) {
			mAudioManager.unregisterMediaButtonEventReceiver(mCn);
			mCn = null;
		}
	}
	...
	// modify mafei@szprize.com  bugid:38109
	public interface Callback {
		void updatespeakerRes(boolean isSpeaker);
	}

...
import com.android.view.FileListView;
...
public class RecordingFileList extends Activity implements
	...
	private void resetUI(int toState) {
		...
		case SoundRecorderService.STATE_IDLE:
			...
			// modify by gyc --- 淇敼閫�鍑烘挱鏀惧拰閫夋嫨妯″紡鏃舵病鏈夋仮澶嶅師濮嬬姸鎬佺殑闂 -- start ---
		...
		case SoundRecorderService.STATE_PLAYING:
			...
			// modify by gyc --- 淇敼杩涘叆鎾斁鍜岄�夋嫨妯″紡鏃舵渶搴曢儴鐨勪竴鏉¤瑕嗙洊闂 -- end ---
		...
		case SoundRecorderService.STATE_PAUSE_PLAYING:
			...
			FrameLayout.LayoutParams params2 = (FrameLayout.LayoutParams) mListView.getLayoutParams();
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if (readExtStorage != PackageManager.PERMISSION_GRANTED) {
            finish();
            return; //prize add by xiarui for bug67885 2018-11-22
        }
		//prize modify by tianhuiju android p soundrecorder error 20180908 start
		...
		setContentView(R.layout.recorder_file_list_layout_notch);
		//prize modify by tianhuiju android p soundrecorder error 20180908 end
		ActionBar actionBar = getActionBar();
		actionBar.hide();
		//prize modify by tianhuiju status background color 20180920 start
		getWindow().setNavigationBarColor(Color.parseColor("#f0f0f0"));
		getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR | View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR);
		//prize modify by tianhuiju status background color 20180920 end
	...
	private /*BaseExtListView*/ FileListView mListView;
	...
	private /*TextView*/ImageView mCancelTv;
	private TextView mNumberTv;
	private /*TextView*/ImageView mSelectTv;
	 ...
	 */
	private void init() {
		mListView = findViewById(R.id.file_list_listview);
		initFileListView();
	...
	private void initFileListView() {
		...
		mSelectTv = findViewById(R.id.tv_select);
	...
	private void updateEnable(int size) {
		...
		if (sum == mList.size()) {
			...
			mSelectTv.setImageResource(R.drawable.ic_unselect_all);
		} else {
			//mSelectTv.setText(getString(R.string.recorder_top_layout_selectall_txt));
			mSelectTv.setImageResource(R.drawable.ic_select_all);
		}
	}
	...
	AlertDialog t = null;
	protected void showRenameOrDeleteAlertDialog(final int type) {
//		final Dialog dialog = new Dialog(this, R.style.Dialog);
		...
		dialog.setPositiveButton(R.string.button_ok,new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				renameFile(newName);
				dialog.dismiss();
				imm.hideSoftInputFromInputMethod(renameETxt.getWindowToken(),0);
			}});
		dialog.setNegativeButton(com.android.internal.R.string.cancel,new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				imm.hideSoftInputFromInputMethod(renameETxt.getWindowToken(),0);
		...
		renameETxt.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start,
				...
				if(t!=null) {
					Button bt =	t.getButton(Dialog.BUTTON_POSITIVE);
					Log.i("","");
	...
	public boolean requestPermissions() {
		...
		//prize modify by tianhuiju write external storage 20180921 start
		if (writeExtStorage != PackageManager.PERMISSION_GRANTED) {
			mPermissionStrings.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
			mRequest = true;
		}
		//prize modify by tianhuiju write external storage 20180921 end
...
//					MediaStore.Audio.Media.DATA + delIds, null);
	...
	private void renameRecorder() {
		...
		if (list.size() == 1) {
			...
			showRenameOrDeleteAlertDialog(1);
		}
	}
		...
		if (files.size() == 1) {
			...
			try {
				if (cursor != null && cursor.moveToFirst() && cursor.getCount() > 0) {
					...
					if(isSim2){
						//prize modify by tianhuiju android p sound recorder error 20180907 start
						/*RingtoneManager.setActualDefaultRingtoneUri(getApplicationContext(),
								RingtoneManager.TYPE_RINGTONE2, newUri);*/
						RingtoneManager.setActualDefaultRingtoneUri(getApplicationContext(),
								RingtoneManager.TYPE_RINGTONE, newUri);
						//prize modify by tianhuiju android p sound recorder error 20180907 end
					}else{
						RingtoneManager.setActualDefaultRingtoneUri(getApplicationContext(),
								RingtoneManager.TYPE_RINGTONE, newUri);
					}
	...
	protected void setSendEnable(int sum) {
		//prize modify by xiarui for bug68616 2018-11-26 start
		//if (sum == 1) {
		if (sum > 0) {
		//prize modify by xiarui for bug68616 2018-11-26 end
	...
	protected void setTopTitleTxt(int sum) {
		if(sum<=0) {
			mNumberTv.setText(getString(R.string.prize_selected));
		}else if (sum<=1){
			mNumberTv.setText(getString(R.string.prize_single_selected_num, String.valueOf(sum)));
		}else {

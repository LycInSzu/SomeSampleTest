...
public class CheckBoxAndSettingsPreference extends CheckBoxPreference {
    ...
    protected void onCheckBoxClicked() {
        ...
        if (key.equals(KEY_ENABLE_CELLBROADCAST)) {
            ...
            if (!isChecked()) {
                mEnableCB = true;
                cbEditor.putBoolean(KEY_ENABLE_CELLBROADCAST, true);
            } else {
                mEnableCB = false;
                cbEditor.putBoolean(KEY_ENABLE_CELLBROADCAST, false);
            }
            cbEditor.commit();
            enableCB();
        }
    ...
    private class CBEnableThread extends Thread {
        ...
        public void run() {
            ...
            if (MtkSmsManager.getDefault().activateCellBroadcastSms(mEnableCB) ||

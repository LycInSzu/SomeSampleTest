...
//add by zhouerlong prize launcher 20180906
                ...
                //prize modified by liyuchong, adapte Condor theme park, 20190218-begin
            if (CondorThemeManager.getInstance().getmShadowBitmap()!=null){
                Log.d(TAG, "---------addIconToDBAndMemCache  ----- CondorThemeManager.getInstance().getmShadowBitmap()!=null  ---------------");
//                entry.icon =ImageUtils.drawableToBitmap(getFullResIcon(app));
//            }else {

                boolean inTheTheme = false;
                if (!isGoogleIcon(app.getComponentName())) {
                    Bitmap newIcon = CondorThemeManager.getInstance().getIcon(app.getComponentName());
                    if (newIcon != null) {
                        f = new BitmapDrawable(newIcon);
                        inTheTheme = true;
                    }
                } else {
                    inTheTheme = true;
                ...
                if (!inTheTheme) {
                    Log.d(TAG, "---------addIconToDBAndMemCache  ----- inTheTheme  is  " +inTheTheme);
//                    if (CondorThemeManager.getInstance().getmShadowBitmap() == null) {
//                        entry.icon = ImageUtils.drawableToBitmap(getFullResIcon(app));
//                    } else
                    f = new BitmapDrawable(ImageUtils.getMaskIcon(ImageUtils.drawableToBitmap(getFullResIcon(app)), CondorThemeManager.getInstance().getmMaskBitmap(), CondorThemeManager.getInstance().getmShadowBitmap()));

//                    entry.icon = ImageUtils.getMaskIcon(ImageUtils.drawableToBitmap(getFullResIcon(app)), CondorThemeManager.getInstance().getmMaskBitmap(), CondorThemeManager.getInstance().getmShadowBitmap());
                }
            }else if (CondorThemeManager.getInstance().getDefaulteIcon(app.getComponentName())!=null){
                    Bitmap defaulteIcon = CondorThemeManager.getInstance().getDefaulteIcon(app.getComponentName());
                    f = new BitmapDrawable(defaulteIcon);
            }
...
//add by zhouerlong prize launcher 20180906
                    ...
                    //prize modified by liyuchong, adapte Condor theme park, 20190218-begin
                    if (CondorThemeManager.getInstance().getmShadowBitmap()!=null) {
                        Log.d(TAG, "---------cacheLocked  ----- CondorThemeManager.getInstance().getmShadowBitmap()!=null  ---------------");

//                        entry.icon =ImageUtils.drawableToBitmap(getFullResIcon(info));
//                    }else

                        boolean inTheTheme = false;
                        if (!isGoogleIcon(componentName)) {
                            Bitmap newIcon = CondorThemeManager.getInstance().getIcon(componentName);
                            if (newIcon != null) {
                                f = new BitmapDrawable(newIcon);
                                inTheTheme = true;
                            }
                        ...
                        if (!inTheTheme) {
                            Log.d(TAG, "---------cacheLocked  ----- inTheTheme  is  " +inTheTheme);

//                            if (CondorThemeManager.getInstance().getmShadowBitmap() == null) {
//                                entry.icon = ImageUtils.drawableToBitmap(getFullResIcon(info));
//                            } else
                                f = new BitmapDrawable(ImageUtils.getMaskIcon(ImageUtils.drawableToBitmap(getFullResIcon(info)), CondorThemeManager.getInstance().getmMaskBitmap(), CondorThemeManager.getInstance().getmShadowBitmap()));
                        }
                    }else if (CondorThemeManager.getInstance().getDefaulteIcon(componentName)!=null){
                        Bitmap defaulteIcon = CondorThemeManager.getInstance().getDefaulteIcon(componentName);
                        f = new BitmapDrawable(defaulteIcon);
                    }
                    //prize modified by liyuchong, adapte Condor theme park, 20190218-end
                    //add by zhouerlong prize launcher 20180906
...
//add by zhouerlong prize launcher 20180906
    ...
    public interface ItemInfoUpdateReceiver {
        ...
        void reapplyItemInfo(ItemInfoWithIcon info);
	                ...
	                //prize modified by liyuchong, adapte Condor theme park, 20190218-begin
    private boolean isGoogleIcon(ComponentName componentName) {
        String pkg = componentName.getPackageName();
        return pkg.contains("google") || pkg.contains("com.android.vending") || pkg.contains("com.android.chrome");
    }
	                //prize modified by liyuchong, adapte Condor theme park, 20190218-end
}

...
/*PRIZE-remove imageview-huangliemin-2016-5-27 -end*/
    ...
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
    	...
    	if(isRecentCard){
    		 ...
    		 PhoneAccountHandle mPhoneAccountHandle = entry.getIntent().getParcelableExtra("android.telecom.extra.SUGGESTED_PHONE_ACCOUNT_HANDLE");
...
//        icon.setVisibility(iconVisibility);
        ...
        if (!TextUtils.isEmpty(entry.getHeader())) {
			...
			//prize add by liyuchong, modify phone number show order in arabic language ,20190124-begin
            if(needLtrDirection(entry.getHeader())){

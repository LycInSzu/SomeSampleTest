...
// prize modified by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
import android.widget.RemoteViews;
import android.view.View;
import android.graphics.BitmapFactory;
import android.graphics.Bitmap.Config;
// prize modified by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-end
...
public class StatusBarNotifier
  ...
  // prize modified by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
  private RemoteViews prize_notificationView;
  // prize modified by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-end

  // prize add by longzhongping, InCallUI, hotLine, 2018.09.04-start
  public static String[] visitHotline = new String[] { "0769 8860 8291",
          ...
          "076988608611"};
  // prize add by longzhongping, InCallUI, hotLine, 2018.09.04-end
  ...
  private void buildAndSendNotification(
    ...
    Log.i("StatusBarNotifier.buildAndSendNotification", "buildAndSendNotification Start");
    ...
    // prize modified by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
    if(notificationType == NOTIFICATION_INCOMING_CALL && !call.isVideoCall()) {
        prize_notificationView = new RemoteViews(context.getPackageName(), R.layout.prize_incall_incoming_notification);
      //prize add by liyuchong, modify phone number show order in arabic language ,20190129-begin
      if (needLtrDirection(contentTitle)) {
		  prize_notificationView.setTextViewText(R.id.incoming_call_number, "\u202D"+contentTitle+"\u202C");
      }else{
		  prize_notificationView.setTextViewText(R.id.incoming_call_number, contentTitle);
	  }
      //prize add by liyuchong, modify phone number show order in arabic language ,20190129-end
        
        prizeCreateIncomingCallNotification(call, callState, callAudioState, builder);
      Log.i("StatusBarNotifier.buildAndSendNotification", "buildAndSendNotification contactInfo.location=" + contactInfo.location);
        prize_notificationView.setTextColor(R.id.incoming_call_style, context.getResources().getColor(R.color.incoming_call_style_color));
        if (null != contactInfo.location && !contactInfo.location.replace(" ", "").equals("")) {
          prize_notificationView.setTextViewText(R.id.incoming_call_style, contactInfo.location);
        } else {
          // add for name display  when  contactInfo.location = null.
          if (!TextUtils.isEmpty(content)) {
            prize_notificationView.setTextViewText(R.id.incoming_call_style, content);
          }
        }

        // prize add by longzhongping, InCallUI, hotLine, 2018.09.027-start
        if (contactInfo.number != null) {
          for (String num : visitHotline) {
            if (contactInfo.number.replaceAll(" ", "").equals(num)) {
              prize_notificationView.setTextViewText(R.id.incoming_call_style, context.getResources().getString(R.string.visit_hotline));
            }
          }
        }
        // prize add by longzhongping, InCallUI, hotLine, 2018.09.027-end
        builder.setCustomBigContentView(prize_notificationView);
        builder.setContent(prize_notificationView);
    ...
    switch (notificationType) {
      ...
      case NOTIFICATION_IN_CALL:
        if (BuildCompat.isAtLeastO()) {
          // prize delete by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
          //publicBuilder.setColorized(true);
          //builder.setColorized(true);
          // prize delete by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-end
          builder.setChannelId(NotificationChannelId.ONGOING_CALL);
        }
        break;
      default:
    ...
    // prize Modified by longzhongping, InCallUI, AndroidP_UI , 2018.09.04-start
    if (contactInfo.photo != null) {
        builder.setLargeIcon(largeIcon);
    }
    // prize Modified by longzhongping, InCallUI, AndroidP_UI , 2018.09.04-end
    ...
    if (!InCallUtils.isTwoIncomingCalls()) {
      ...
      // prize add by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
        //createIncomingCallNotification(call, callState, callAudioState, builder);
        if(notificationType != NOTIFICATION_INCOMING_CALL){
          createIncomingCallNotification(call, callState, callAudioState, builder);
        }
        // prize add by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-end
    ...
    if (dialerRingtoneManager.shouldPlayCallWaitingTone(callState)) {
      Log.v("StatusBarNotifier.buildAndSendNotification", "playing call waiting tone");
  ...
  // prize add by longzhongping, InCallUI, Incoming Call Notifycation, 2018.09.27-start
  private void prizeCreateIncomingCallNotification(
    ...
    if (state == DialerCall.State.ACTIVE
      ...
      /// @}
      prize_notificationView.setViewVisibility(R.id.incall_incoming_accept_button, View.GONE);
      //prize_notificationView.setViewVisibility(R.id.incall_incoming_call_time, View.VISIBLE);
      PendingIntent hangupPendingIntent = createNotificationPendingIntent(context, ACTION_HANG_UP_ONGOING_CALL);
      prize_notificationView.setOnClickPendingIntent(R.id.incall_incoming_descline_button, hangupPendingIntent);
      //addHangupAction(builder);
      //**addSpeakerAction(builder, callAudioState);
    } else if (state == DialerCall.State.INCOMING || state == DialerCall.State.CALL_WAITING) {

      prize_notificationView.setViewVisibility(R.id.incall_incoming_accept_button, View.VISIBLE);
      //addDismissAction(builder);
      PendingIntent declinePendingIntent = createNotificationPendingIntent(context, ACTION_DECLINE_INCOMING_CALL);
      prize_notificationView.setOnClickPendingIntent(R.id.incall_incoming_descline_button, declinePendingIntent);
      if (call.isVideoCall()) {
        PendingIntent answerVideoPendingIntent = createNotificationPendingIntent(context, ACTION_ANSWER_VIDEO_INCOMING_CALL);
        prize_notificationView.setOnClickPendingIntent(R.id.incall_incoming_accept_button, answerVideoPendingIntent);
        //addVideoCallAction(builder);
        //**addVoiceAction(builder);
      } else {
        PendingIntent answerVoicePendingIntent = createNotificationPendingIntent(context, ACTION_ANSWER_VOICE_INCOMING_CALL);
        prize_notificationView.setOnClickPendingIntent(R.id.incall_incoming_accept_button, answerVoicePendingIntent);
  ...
  private void createIncomingCallNotification(
    ...
    if (state == DialerCall.State.ACTIVE
        ...
        || DialerCall.State.isDialing(state)) {
      addHangupAction(builder);
      // prize add by longzhongping, InCallUI, for BUG:68269, 2018.11.26-start
      //addSpeakerAction(builder, callAudioState);
      if(!call.isVideoCall()){ addSpeakerAction(builder, callAudioState);}
      // prize add by longzhongping, InCallUI, for BUG:68269, 2018.11.26-end
    } else if (state == DialerCall.State.INCOMING || state == DialerCall.State.CALL_WAITING) {
      addDismissAction(builder);
      if (call.isVideoCall()) {
    ...
    // Should give user another chance to go back to incall screen. Also show statusbar notifier
    // when call is in connecting status. @{
    } else if (DialerCall.State.isConnecting(state)) {
      addHangupAction(builder);
    }
    /// @}
  }
  ...
  String getContentTitle(ContactCacheEntry contactInfo, DialerCall call) {
    ...
    boolean isIncomingOrWaiting = call.getState() == DialerCall.State.INCOMING
        || call.getState() == DialerCall.State.CALL_WAITING;
    /// @}
  ...
  public int getIconToDisplay(DialerCall call) {
    ...
    if (call.getState() == DialerCall.State.ONHOLD) {
      return R.drawable.prize_quantum_ic_phone_paused_vd_theme_24;
    } else if (call.getVideoTech().getSessionModificationState()
            == SessionModificationState.RECEIVED_UPGRADE_TO_VIDEO_REQUEST
        || call.isVideoCall()) {
      return R.drawable.prize_quantum_ic_videocam_white_24;
      ...
      return R.drawable.ic_ongoing_phone_24px_01;
    }

...
public class ContactGridManager {
  ...
  private final TextView prizelabelTextView;
  ...
  public ContactGridManager(
    ...
    // prize Modified by longzhongping, InCallUI, AndroidP_UI , 2018.09.04-start
    prizelabelTextView = view.findViewById(R.id.prize_contactgrid_lable);
    // prize Modified by longzhongping, InCallUI, AndroidP_UI , 2018.09.04-end
   ...
   */
  /// M: Change to public
  /// Google code: private void updateTopRow() {
  ...
  private void updatePrimaryNameAndPhoto() {
    if (TextUtils.isEmpty(primaryInfo.name())) {
	  ...
	  //prize add by liyuchong, modify phone number show order in arabic language ,20190124-begin
	  String numberName=primaryInfo.name();
	  if(needLtrDirection(numberName)){
		  nameDirection = View.TEXT_DIRECTION_LTR;
	  }
	  //prize add by liyuchong, modify phone number show order in arabic language ,20190124-end
    ...
    if (avatarImageView != null) {
      if (hideAvatar) {
        ...
        if (hasPhoto) {
          ...
          if(!TextUtils.isEmpty(primaryInfo.name()) && !primaryInfo.nameIsNumber()){
            if (TextUtils.isEmpty(primaryInfo.number()) || PhoneNumberUtils.isLocalEmergencyNumber(context, primaryInfo.number())) {
              avatarImageView.setBackgroundResource(R.drawable.prize_contact_default_photo);
            } else {
              avatarImageView.setBackground(letterTile);
            }
          }else{
  ...
  private void updateBottomRow() {
    ...
    if (hdIconImageView.getVisibility() == View.GONE) {
      if (info.isHdAttemptingIconVisible) {
        ...
        hdIconImageView.setImageDrawable(context.getDrawable(R.drawable.quantum_ic_hd_vd_theme_24));
        /// @}
        hdIconImageView.setVisibility(View.VISIBLE);
    ...
    /// @}
      ...
      if (!isTimerStarted) {
        ...
        LogUtil.i(
                "ContactGridManager.updateBottomRow",
                "primaryInfo.location()=%s",
                primaryInfo.location());
        if(null != prizelabelTextView) {
          if (!TextUtils.isEmpty(primaryInfo.location())) {
            prizelabelTextView.setText(primaryInfo.location());
            prizelabelTextView.setVisibility(View.VISIBLE);
          } else {
            prizelabelTextView.setVisibility(View.GONE);
          }
        }
        // prize Modified by longzhongping, InCallUI, AndroidP_UI , 2018.09.04-end

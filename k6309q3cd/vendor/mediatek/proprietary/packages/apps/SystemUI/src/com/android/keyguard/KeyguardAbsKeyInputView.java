...
public abstract class KeyguardAbsKeyInputView extends LinearLayout
    ...
    //prize-yuyulong-20180908 lock password failed attempts start
    private int i=0;
    private int fivecount=5;
    private int remaincount;
    boolean passw_pin_flag = false;//prize-yuyulong-20180913
    //prize-yuyulong-20180908 lock password failed attempts end
    ...
    protected void verifyPasswordAndUnlock() {
        ...
        if (entry.length() <= MINIMUM_PASSWORD_LENGTH_BEFORE_REPORT) {
            ...
            passw_pin_flag = true;//prize-yuyulong-20180913 lock password failed attempts				
    ...
    private void onPasswordChecked(int userId, boolean matched, int timeoutMs,
        ...
        if (matched) {
            ...
            if (dismissKeyguard) {
                mDismissing = true;
                //prize-add for bugid:69013 -by-peisaisai-20190103-begin
                hideSoftInputFromWindow();
                //prize-add for bugid:69013 -by-peisaisai-20190103-end
            ...
            if (isValidPassword) {
                ...
                if (timeoutMs > 0) {
                    i=0;//prize-yuyulong-20180908 lock password failed attempts				
		  ...
		  //prize-yuyulong-20180908 lock password failed attempts start		  
			if(passw_pin_flag){
				String str = mContext.getString(getWrongPasswordStringId());
				if( "".equals(str)){
					mSecurityMessageDisplay.setMessage(R.string.kg_wrong_password);
				}
				if(str.contains("Wrong PIN")){
					mSecurityMessageDisplay.setMessage(R.string.kg_wrong_pin);
				}else if(str.contains("Wrong Password")){
					mSecurityMessageDisplay.setMessage(R.string.kg_wrong_password);
				}else{
				  	mSecurityMessageDisplay.setMessage(R.string.kg_wrong_password);
				}
				passw_pin_flag = false;
				//mSecurityMessageDisplay.setMessage(getWrongPasswordStringId());
			}else{
				i++;
				if(i <5){
				remaincount = fivecount-i;
				}
				if(remaincount != 0){
				mSecurityMessageDisplay.formatMessage(getWrongPasswordStringId(), remaincount);
				}else{
					mSecurityMessageDisplay.setMessage(getWrongPasswordStringId());
				}				
			}
    ...
    //prize-add for bugid:69013 -by-peisaisai-20190103-begin
    protected abstract void hideSoftInputFromWindow();
    //prize-add for bugid:69013 -by-peisaisai-20190103-end
